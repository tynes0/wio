...
int main()
{
    ...
    bool ok = true;
    Lexer lexer("fn Count(limit: i32) -> i32 { "
                "  mut index: i32; "
                "  mut total: i32; "
                "  while (index < limit) { "
                "    index += 1; "
    ...
    ok &= expect(typedText.find("\"while.header\"") != std::string::npos &&
                     ...
                     typedText.find("local-place \"index\"") != std::string::npos &&
                     typedText.find("local-place \"total\"") != std::string::npos,
                 "Typed WIR printer must expose deterministic loop blocks and mutable places");
    ...
    count.blocks.front().instructions.back().operands.push_back(count.blocks.front().instructions.front().result);
    const typed::VerificationResult malformedResult = typed::Verifier{}.verify(malformed);
    ok &=
        expect(hasCode(malformedResult, "WIR1417"), "Typed WIR verifier must reject unexpected loop branch arguments");
    ...
    typed::BasicBlock* exitBlock = nullptr;
    for (typed::BasicBlock& block : nonDominatingCount.blocks)
    {
        if (block.name == "while.body")
            bodyBlock = &block;
        else if (block.name == "while.exit")
            exitBlock = &block;
    }
    if (bodyBlock && exitBlock && !bodyBlock->instructions.empty() && bodyBlock->instructions.front().result &&
        !exitBlock->instructions.empty() && !exitBlock->instructions.back().operands.empty())
    {
        exitBlock->instructions.back().operands.front() = bodyBlock->instructions.front().result;
    }
    const typed::VerificationResult dominanceResult = typed::Verifier{}.verify(nonDominating);
    ok &= expect(hasCode(dominanceResult, "WIR1421"),
