...
int main()
{
    ...
    bool ok = true;
    Lexer lexer("attribute app_tag(name: string) for application retain runtime; "
    ...
    // responsible for std::async. Remove that synthetic import and keep the
    // parser-produced application metadata while replacing the generated host
    // loop with a self-contained entry.
    std::erase_if(program->statements,
    ...
    if (FunctionDeclaration* entry = findApplicationEntry(program))
    {
        ...
        statements.push_back(makeNodePtr<ReturnStatement>(
            makeNodePtr<IntegerLiteral>(Token{.type = TokenType::integerLiteral, .value = "0", .loc = location}),
            location));
    ...
    {
        applicationAst->attributes.front()->processorBindings.push_back(
            AttributeStatement::ProcessorBinding{.canonicalTypeName = "demo::GuardProcessor",
                                                 .phase = "validation",
                                                 .hookCppName = "ValidateApplication",
                                                 .hookMode = "metadata"});
    }

    typed::BuildResult build =
        typed::Builder{}.build(program, typed::BuildOptions{.logicalModuleName = "example.application_metadata"});
    if (!build.succeeded())
    ...
    if (module.contract.application)
    {
        ...
        ok &= expect(application.stages[0].runs.size() == 1 && application.stages[0].runs[0].targetName == "motion" &&
        ...
        ok &= expect(application.stages[1].runs.size() == 1 && application.stages[1].runs[0].applicationTarget,
                     "Application update must remain an explicit schedule run");
    }

    const auto appAttribute =
        std::ranges::find_if(module.contract.attributes, [](const AttributeApplicationDescriptor& attribute)
                             { return attribute.canonicalName.find("app_tag") != std::string::npos; });
    ok &=
        expect(appAttribute != module.contract.attributes.end() && appAttribute->runtimeRetained &&
                   appAttribute->targetKind == MetadataTargetKind::Application && appAttribute->arguments.size() == 1 &&
                   appAttribute->arguments[0].sourceText == "demo" && appAttribute->processors.size() == 1 &&
                   appAttribute->processors[0].phase == AttributeProcessorPhase::Validation,
    ...
    const auto worldReflection =
        std::ranges::find_if(module.contract.reflection,
                             [](const ReflectionDescriptor& descriptor) { return descriptor.logicalName == "World"; });
    ok &= expect(worldReflection != module.contract.reflection.end() && worldReflection->fields.size() == 1 &&
                     worldReflection->fields[0].name == "value" && worldReflection->fields[0].isMutable &&
                     worldReflection->fields[0].attributes.size() == 1,
                 "Reflection must expose field layout and its applied attributes");
    const auto modeReflection =
        std::ranges::find_if(module.contract.reflection,
                             [](const ReflectionDescriptor& descriptor) { return descriptor.logicalName == "Mode"; });
    ok &=
        expect(modeReflection != module.contract.reflection.end() && modeReflection->cases.size() == 2 &&
    ...
    ok &= expect(typedText.find("application \"Demo\"") != std::string::npos &&
                     ...
                     typedText.find("phase=validation") != std::string::npos,
