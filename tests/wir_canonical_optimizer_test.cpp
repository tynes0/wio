...
namespace
{
    ...
    wio::wir::lowered::Module makeOptimizationModule()
    {
        ...
        const TypeId arrayType = module.types.intern(
            Type{.kind = TypeKind::Array, .arguments = {module.types.i32Type()}, .staticExtent = 2});
        const TypeId dynamicArrayType =
            module.types.intern(Type{.kind = TypeKind::Array, .arguments = {module.types.i32Type()}});
        const TypeId callableType =
            module.types.intern(Type{.kind = TypeKind::Function, .arguments = {module.types.i32Type()}});
        ...
        entry.id = BlockId{0};
        entry.name = "entry";
        entry.instructions = {lowered::Instruction{.opcode = lowered::Opcode::Constant,
                                                   .result = ValueId{0},
                              ...
                              lowered::Instruction{.opcode = lowered::Opcode::Binary,
                                                   .result = ValueId{3},
                                                   .resultType = module.types.i32Type(),
                                                   .operands = {ValueId{1}, ValueId{2}},
                                                   .binaryOperator = typed::BinaryOperator::Add},
                              lowered::Instruction{.opcode = lowered::Opcode::Unary,
                                                   .result = ValueId{4},
                                                   .resultType = module.types.i32Type(),
                                                   .operands = {ValueId{3}},
                              ...
                              lowered::Instruction{.opcode = lowered::Opcode::ArrayCreate,
                                                   .result = ValueId{6},
                                                   .resultType = arrayType,
                                                   .operands = {ValueId{1}, ValueId{2}}},
                              lowered::Instruction{.opcode = lowered::Opcode::ArrayGet,
                                                   .result = ValueId{7},
                                                   .resultType = module.types.i32Type(),
                              ...
                              lowered::Instruction{.opcode = lowered::Opcode::CondJump,
                                                   .operands = {ValueId{0}},
                                                   .targets = {lowered::BranchTarget{.block = BlockId{1}},
        ...
        selected.instructions = {
            lowered::Instruction{.opcode = lowered::Opcode::Jump,
                                 .targets = {lowered::BranchTarget{.block = BlockId{3}, .arguments = {ValueId{3}}}}}};
        ...
        merge.id = BlockId{3};
        merge.name = "merge";
        merge.parameters = {lowered::Parameter{.id = ValueId{8}, .name = "forwarded", .type = module.types.i32Type()}};
        merge.instructions = {lowered::Instruction{.opcode = lowered::Opcode::Return, .operands = {ValueId{8}}}};

        function.blocks = {std::move(entry), std::move(selected), std::move(unreachable), std::move(merge)};
        module.functions.push_back(std::move(function));

        const TypeId i8Type = module.types.intern(Type{.kind = TypeKind::I8});
        const auto appendUnsafeArithmetic = [&](const FunctionId id, std::string name, const TypeId type,
                                                typed::Literal left, typed::Literal right,
                                                const typed::BinaryOperator operation)
        {
            ...
            unsafe.callableType = module.types.intern(Type{.kind = TypeKind::Function, .arguments = {type}});
        ...
        appendUnsafeArithmetic(FunctionId{1}, "PreserveOverflow", i8Type, std::int64_t{127}, std::int64_t{1},
                               typed::BinaryOperator::Add);
        appendUnsafeArithmetic(FunctionId{2}, "PreserveDivisionByZero", module.types.i32Type(), std::int64_t{5},
                               std::int64_t{0}, typed::BinaryOperator::Divide);
        return module;
    }
    ...
    wio::wir::typed::Module makePipelineModule()
    {
        ...
        entry.instructions = {typed::Instruction{.opcode = typed::Opcode::Constant,
                              ...
                              typed::Instruction{.opcode = typed::Opcode::Constant,
                                                 ...
                                                 .resultType = module.types.i32Type(),
                                                 .literal = std::int64_t{1}},
                              typed::Instruction{.opcode = typed::Opcode::ArrayCreate,
                                                 .result = ValueId{4},
                                                 .resultType = arrayType,
                                                 .operands = {ValueId{0}, ValueId{1}}},
                              typed::Instruction{.opcode = typed::Opcode::ArrayGet,
                                                 .result = ValueId{5},
                                                 .resultType = module.types.i32Type(),
                                                 .operands = {ValueId{4}, ValueId{3}}},
                              typed::Instruction{.opcode = typed::Opcode::Return, .operands = {ValueId{5}}}};
...
int main()
{
    ...
    ok &= expect(statistics.constantsFolded == 2, "Binary and dependent unary constants must fold deterministically");
    ok &= expect(statistics.valuesPropagated == 1, "Common trivial block arguments must be propagated");
    ...
    ok &= expect(statistics.instructionsRemoved >= 2, "Dead folded values and obsolete conditions must be eliminated");
    ok &= expect(statistics.heapAllocations == 2, "Array backing storage must receive explicit heap decisions");
    ...
    ok &= expect(pipeline.completedPasses() ==
