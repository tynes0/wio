...
namespace wio::wir
{
    std::vector<std::string> lowerHierarchy(lowered::Module& module)
    {
        ...
        for (std::size_t index = 0; index < module.types.size(); ++index)
        {
            ...
            if (type.nominalKind != NominalKind::Object && type.nominalKind != NominalKind::Interface) continue;
            ...
            {
                if (active.contains(current)) { errors.push_back("Cyclic object/interface hierarchy: " + type.name); return; }
                if (!seen.insert(current).second) return;
                const auto* ancestor = module.types.tryGet(current);
                if (!ancestor) { errors.push_back("Unknown hierarchy type: " + type.name); return; }
                active.insert(current);
                type.castTypes.push_back(current);
                for (TypeId base : ancestor->baseTypes) visit(base);
            ...
            for (TypeId contract : type.castTypes)
                ...
                {
                    const auto found = std::ranges::find_if(type.methods, [&](const MethodLayout& candidate)
                    {
                        return candidate.name == method.name && candidate.parameterTypes == method.parameterTypes &&
                    ...
                    if (found == type.methods.end())
                        ...
                        type.dispatchEntries.push_back({contract, method.slot, found->isAbstract ? FunctionId{} : found->function});
                }
            for (const auto& function : module.functions)
                if (function.ownerType == id)
                {
                    ...
                    {
                        const auto origin = std::ranges::find(module.functions, function.genericOrigin, &lowered::Function::id);
                        if (origin != module.functions.end()) name = origin->name;
                    }
                    if (name == "OnDestruct" || name.ends_with("::OnDestruct")) type.destructor = function.id;
        ...
        for (auto& function : module.functions)
        {
            ...
            for (const auto& parameter : function.parameters) values[parameter.id] = parameter.type;
            for (const auto& block : function.blocks)
            {
                for (const auto& parameter : block.parameters) values[parameter.id] = parameter.type;
                for (const auto& instruction : block.instructions) if (instruction.result) values[instruction.result] = instruction.resultType;
            }
            for (auto& block : function.blocks)
                for (auto& instruction : block.instructions)
                {
                    if (instruction.opcode != lowered::Opcode::FieldPlace || instruction.operands.size() != 1) continue;
                    ...
                    {
                        if (!wrappers.insert(receiver).second) { errors.push_back("Cyclic field receiver reference type"); break; }
                        if (type->kind != TypeKind::Reference || type->arguments.empty()) break;
                    ...
                    std::function<bool(TypeId)> resolve = [&](TypeId owner)
                    {
                        ...
                        for (TypeId base : type->baseTypes) if (resolve(base)) return true;
                        return false;
                    };
                    if (!resolve(receiver)) errors.push_back("Field has no canonical storage: " + instruction.selector);
                }
        }
        return errors;
    }
