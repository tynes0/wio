    ...
    VerificationResult Verifier::verify(const Module& module) const
    {
        ...
        for (const Function& function : module.functions)
        {
            ...
            if (function.isAsync && (!validAsyncResult || !function.coroutine ||
                ...
                report("LIR1114",
                       "Async Lowered WIR function requires coroutine<T> return and matching coroutine layout.",
                       function.source, function.id);
            if (!function.isAsync && function.coroutine)
                report("LIR1115", "Non-async Lowered WIR function cannot carry coroutine layout.", function.source,
                       function.id);
            ...
            if (!callableType || callableType->kind != TypeKind::Function || callableType->arguments.empty() ||
                ...
                report("LIR1110", "Lowered WIR function requires a callable type ending in its return type.",
                       function.source, function.id);
            else
            {
                ...
                if (!signatureMatches)
                    report("LIR1112",
                           "Lowered WIR visible parameters must match the callable type after hidden receiver/capture "
                           "parameters.",
                           function.source, function.id);
            }
            if (function.captureParameterCount != function.captures.size() ||
                ...
                report("LIR1111", "Lowered WIR closure capture metadata is inconsistent.", function.source,
                       function.id);
            if (function.isMethod)
            {
                ...
                if (!owner || owner->kind != TypeKind::Named || !receiver || receiver->kind != TypeKind::Reference ||
                    ...
                    report("LIR1106", "Lowered WIR methods require a named owner and leading self parameter.",
                           function.source, function.id);
            }
            if (function.nativeBinding)
            {
                ...
                bool valid = function.isExternal && !function.isAbstract && !binding.symbol.empty() &&
                             ...
                             binding.result.type == function.returnType &&
                             validNativeAbiValue(module.types, binding.result) &&
                ...
                if (!valid)
                    report("LIR1113",
                           "Native Lowered WIR function binding has an invalid identity, ABI signature, or thunk "
                           "contract.",
                           function.source, function.id);
            }
        }
        ...
        if (std::ranges::any_of(module.types.types(), [](const Type& type) { return !type.castTypes.empty(); }))
        {
            ...
            for (const auto& error : hierarchyErrors)
                report("LIR1530", error);
                ...
                for (std::size_t b = 0; b < module.functions[f].blocks.size(); ++b)
                    for (std::size_t i = 0; i < module.functions[f].blocks[b].instructions.size(); ++i)
                    {
                        ...
                        if (actual.opcode == Opcode::FieldPlace &&
                            (actual.targetType != canonical.targetType ||
                             actual.projectionIndex != canonical.projectionIndex))
                            report(
                                "LIR1532",
                                "Field projection must identify its canonical declaring subobject and storage index.",
                                actual.source);
                    }
        }
        ...
        if (module.contract.callTable.descriptorVersion != ModuleAbiDescriptorVersion ||
            ...
            report("LIR1501",
                   "Lowered WIR SDK call table version, identity, and export cardinality must be canonical.");
        ...
        if (!lifecycleKnown(lifecycle.apiVersion) || !lifecycleKnown(lifecycle.load) ||
            ...
            report("LIR1503",
                   "Lowered WIR module lifecycle must reference known functions and pair save/restore state hooks.");
        ...
        for (const ReflectionDescriptor& descriptor : module.contract.reflection)
        {
            ...
            if (descriptor.stableTypeId == 0 || descriptor.logicalName.empty() || !type ||
                ...
                report("LIR1505",
                       "Lowered WIR reflection descriptors require unique identities and matching nominal types.");
        ...
        for (const ReflectionDescriptor& descriptor : module.contract.reflection)
        {
            ...
            for (const ReflectedCaseDescriptor& enumCase : descriptor.cases)
                if (!attributesKnown(enumCase.attributes))
                    report("LIR1518",
                           "Lowered WIR reflected enum/flagset case references an unknown attribute application.");
        }
        ...
        for (const Function& function : module.functions)
        {
            ...
            if (function.isExternal)
            {
                if (!function.blocks.empty())
                    report("LIR1104", "External Lowered WIR functions cannot contain blocks.", function.source,
                           function.id);
                continue;
            }
            if (function.blocks.empty())
            {
                report("LIR1105", "Defined Lowered WIR functions require at least one block.", function.source,
                       function.id);
                continue;
            }
            ...
            for (const BasicBlock& block : function.blocks)
            {
                ...
                for (const Instruction& instruction : block.instructions)
                {
                    ...
                    {
                        defineValue(Parameter{.id = instruction.result,
            ...
            if (function.coroutine)
            {
                ...
                const ValueId receiver =
                    function.isMethod && !function.parameters.empty() && owner &&
                            (owner->nominalKind == NominalKind::Object || owner->nominalKind == NominalKind::Interface)
                        ? function.parameters.front().id
                        : ValueId{};
                if (function.coroutine->retainedReceiver != receiver)
                    report("LIR1533", "Coroutine object receiver must be retained by its canonical frame.",
                           function.source, function.id);
                ...
                for (const CoroutineFrameSlot& slot : function.coroutine->frameSlots)
                {
                        ...
                        type->cleanup != slot.cleanup)
                    {
                        report(
                            "LIR1451",
                            "Coroutine frame slots require unique dense ids, values, and matching ownership metadata.",
                            function.source, function.id);
                    }
                }
                for (std::size_t stateIndex = 0; stateIndex < function.coroutine->states.size(); ++stateIndex)
                {
                    ...
                    {
                        report("LIR1452",
                               "Coroutine states require dense ids and valid suspend/resume/value references.",
                               function.source, function.id);
                    }
                }
            }
            ...
            for (const BasicBlock& block : function.blocks)
            {
                ...
                for (std::size_t index = 0; index < block.instructions.size(); ++index)
                {
                    ...
                    {
                        report(terminator ? "LIR1303" : "LIR1304",
                    ...
                    for (const ValueId operand : instruction.operands)
                    {
                        if (!valueType(operand))
                            report("LIR1400", "Lowered WIR instruction references an unknown value.",
                                   instruction.source, function.id, block.id);
                    }
                    ...
                    if (!instruction.expandedOperands.empty())
                    {
                        ...
                        if (!validExpansion)
                            report("LIR1481",
                                   "Lowered WIR pack expansion metadata must align with symbolic pack operands.",
                                   instruction.source, function.id, block.id);
                    }

                    bool callReturnsVoid = false;
                    const bool isCallInstruction =
                        instruction.opcode == Opcode::Call || instruction.opcode == Opcode::NativeInvoke ||
                    ...
                    if (shouldHaveResult && (!instruction.result || !module.types.tryGet(instruction.resultType)))
                        report("LIR1401", "Value-producing Lowered WIR instruction requires a typed result.",
                               instruction.source, function.id, block.id);
                    if (!shouldHaveResult && !isCallInstruction && instruction.result)
                        report("LIR1402", "Lowered WIR terminator cannot produce a value.", instruction.source,
                               function.id, block.id);
                    const bool asyncMetadataCarrier =
                        isCallInstruction || instruction.opcode == Opcode::CancellationCheck ||
                        instruction.opcode == Opcode::CoroutineSuspend || instruction.opcode == Opcode::CoroutineResume;
                    ...
                    {
                        report("LIR1453", "Lowered WIR async metadata is attached to an unsupported instruction.",
                               instruction.source, function.id, block.id);
                    }

                    const bool storageCarrier =
                        instruction.opcode == Opcode::LocalPlace || instruction.opcode == Opcode::ConstructComponent ||
                        instruction.opcode == Opcode::ConstructObject || instruction.opcode == Opcode::ClosureCreate ||
                        instruction.opcode == Opcode::ArrayCreate || instruction.opcode == Opcode::DictionaryCreate ||
                        instruction.opcode == Opcode::Interpolate || instruction.opcode == Opcode::AnyBox ||
                        instruction.opcode == Opcode::NullableWrap || instruction.opcode == Opcode::IteratorCreate;
                    const Type* storageType =
                        instruction.result ? module.types.tryGet(instruction.resultType) : nullptr;
                    const bool requiresHeapStorage =
                        storageCarrier &&
                        (instruction.opcode == Opcode::ConstructObject || instruction.opcode == Opcode::ArrayCreate ||
                         instruction.opcode == Opcode::DictionaryCreate || instruction.opcode == Opcode::Interpolate ||
                         instruction.opcode == Opcode::AnyBox || instruction.opcode == Opcode::IteratorCreate ||
                        ...
                        (requiresHeapStorage && instruction.storageClass != StorageClass::Heap))
                    {
                        report("LIR1483",
                               "Lowered WIR storage and escape metadata is incompatible with " +
                                   ...
                                   " (storage=" + std::string(storageClassName(instruction.storageClass)) +
                                   ", escape=" + std::string(escapeClassName(instruction.escapeClass)) + ").",
                               instruction.source, function.id, block.id);
                    }

                    const bool checkedArrayAccess =
                        instruction.opcode == Opcode::ArrayGet || instruction.opcode == Opcode::ArrayPlace;
                    ...
                    {
                        report("LIR1484",
                               "Lowered WIR bounds-check metadata must appear on every array access and nowhere else.",
                               instruction.source, function.id, block.id);
                    }
                    if (checkedArrayAccess && instruction.boundsCheck != BoundsCheckMode::Required &&
                        instruction.boundsCheck != BoundsCheckMode::NotApplicable)
                    {
                        ...
                        const Type* baseType =
                            proofValid ? module.types.tryGet(valueType(instruction.operands[0])) : nullptr;
                        ...
                        if (!proofValid)
                            report("LIR1485",
                                   "Lowered WIR eliminated bounds check requires a reproducible static or construction "
                                   "proof.",
                                   instruction.source, function.id, block.id);
                    }
                    ...
                    if (instruction.opcode == Opcode::GenericConstant)
                    {
                        ...
                        if (!parameter || parameter->kind != TypeKind::ConstGenericParameter ||
                            ...
                            report("LIR1490", "Generic constant must identify a typed const parameter.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::DefaultValue)
                    {
                        ...
                        if (!type || type->kind == TypeKind::Void || type->kind == TypeKind::Reference ||
                            ...
                            report("LIR1491", "Default value requires a value type and no operands.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::Unary)
                    {
                        if (instruction.operands.size() != 1 ||
                            ...
                            report("LIR1403", "Lowered WIR unary operand must match its result type.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::Binary)
                    {
                        if (instruction.operands.size() != 2 ||
                            ...
                            report("LIR1404", "Lowered WIR binary operands must have the same type.",
                                   instruction.source, function.id, block.id);
                        else if (isComparison(instruction.binaryOperator) &&
                                 instruction.resultType != module.types.boolType())
                            report("LIR1405", "Lowered WIR comparison result must be bool.", instruction.source,
                                   function.id, block.id);
                        else if (!isComparison(instruction.binaryOperator) &&
                                 instruction.resultType != valueType(instruction.operands[0]))
                            report("LIR1406", "Lowered WIR binary result must match its operand type.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::RangeContains)
                    {
                        if (instruction.operands.size() != 3 ||
                            ...
                            report("LIR1482",
                                   "Lowered WIR range containment requires three equal numeric operands and a bool "
                                   "result.",
                                   instruction.source, function.id, block.id);
                    }
                    ...
                    else if (instruction.opcode == Opcode::VariantTest)
                    {
                        ...
                        {
                            report("LIR1418",
                                   "Lowered WIR variant test requires one named value, a variant selector, and a bool "
                                   "result.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::VariantPayload)
                    {
                        ...
                        {
                            report("LIR1419",
                                   "Lowered WIR variant payload requires one named value and a variant selector.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::ArrayLength)
                    {
                        ...
                        {
                            report("LIR1420", "Lowered WIR array length requires one array value and a usize result.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::ArrayElement)
                    {
                        ...
                        {
                            report("LIR1421",
                                   "Lowered WIR array element projection must return its array element type.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::ArrayCreate)
                    {
                        ...
                        if (!valid)
                            report("LIR1422",
                                   "Lowered WIR array creation requires element operands matching its array type and "
                                   "extent.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::ArrayGet)
                    {
                        ...
                        {
                            report("LIR1423",
                                   "Lowered WIR array get requires an array, an integer index, and its element result "
                                   "type.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::DictionaryCreate)
                    {
                        ...
                        if (!valid)
                            report("LIR1437",
                                   "Lowered WIR dictionary creation requires alternating key/value operands matching "
                                   "its concrete type.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::DictionaryGet)
                    {
                        ...
                        {
                            report("LIR1438",
                                   "Lowered WIR dictionary get requires matching dictionary, key, and mapped result "
                                   "types.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::Interpolate)
                    {
                        ...
                        {
                            report("LIR1439",
                                   "Lowered WIR interpolation requires string/text identity, segments, and concrete "
                                   "signatures.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::EnumConstant)
                    {
                             ...
                             resultType->nominalKind != NominalKind::Flagset))
                        {
                            report(
                                "LIR1440",
                                "Lowered WIR enum constant requires a named enum/flagset result and stable selector.",
                                instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::IntrinsicCall)
                    {
                        ...
                        {
                            report("LIR1441",
                                   "Lowered WIR intrinsic call requires a family, selector, receiver, target, and "
                                   "concrete signature.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::AnyBox)
                    {
                        ...
                        {
                            report("LIR1442", "Lowered WIR any box requires one operand and its concrete source type.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::AnyCheckedCast || instruction.opcode == Opcode::AnyTypeTest)
                    {
                        ...
                        {
                            report("LIR1443",
                                   "Lowered WIR any test/cast requires one any operand and a concrete target.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    ...
                    else if (instruction.opcode == Opcode::IteratorCreate)
                    {
                        ...
                        bool valid = iterator && iterator->kind == TypeKind::Iterator &&
                                     ...
                                     instruction.signatureTypes.size() == instruction.operands.size();
                        for (std::size_t argumentIndex = 0; valid && argumentIndex < instruction.operands.size();
                             ++argumentIndex)
                        {
                        ...
                        if (!valid)
                            report("LIR1445", "Lowered WIR iterator creation requires canonical source metadata.",
                                   instruction.source, function.id, block.id);
                    }
                             ...
                             instruction.opcode == Opcode::IteratorValue)
                    {
                        ...
                        if (!iterator || iterator->kind != TypeKind::Iterator || !validResult)
                            report("LIR1446",
                                   "Lowered WIR iterator operation requires one iterator and a canonical result.",
                                   instruction.source, function.id, block.id);
                    }
                             ...
                             instruction.opcode == Opcode::ResultPropagate)
                    {
                        ...
                        if (!valid)
                            report("LIR1479",
                                   "Lowered WIR Result operation requires canonical Result<T> source and "
                    ...
                    else if (instruction.opcode == Opcode::LocalPlace)
                    {
                        ...
                        {
                            report("LIR1424",
                                   "Lowered WIR local place requires a named reference result and no operands.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                             ...
                             instruction.opcode == Opcode::Replace)
                    {
                        ...
                        {
                            report(instruction.opcode == Opcode::PlaceInit ? "LIR1425" : "LIR1426",
                                   instruction.opcode == Opcode::PlaceInit
                                       ? "Lowered WIR place initialization requires a reference place and a matching "
                                         "value."
                        ...
                        if (instruction.opcode == Opcode::PlaceInit && instruction.operands.size() == 2)
                        {
                            ...
                            {
                                report("LIR1431",
                                       "Lowered WIR place initialization must target an uninitialized local place "
                                       "exactly once.",
                                       instruction.source, function.id, block.id);
                            }
                        }
                    ...
                    else if (instruction.opcode == Opcode::Load)
                    {
                        ...
                        {
                            report("LIR1427",
                                   "Lowered WIR load requires a reference place and its referred result type.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::FieldPlace)
                    {
                        ...
                        {
                            report("LIR1428",
                                   "Lowered WIR field place requires a named base, field selector, and "
                                   "non-strengthening reference result.",
                                   instruction.source, function.id, block.id);
                        }
                        else
                        {
                                ...
                                (placeType->isMutable && !field->isMutable))
                            {
                                report(
                                    "LIR1432",
                                    "Lowered WIR field place must match the nominal field layout and field mutability.",
                                    instruction.source, function.id, block.id);
                            }
                        }
                    }
                    else if (instruction.opcode == Opcode::ArrayPlace)
                    {
                        ...
                        {
                            report("LIR1429",
                                   "Lowered WIR array place requires an array reference, integer index, and "
                    ...
                    else if (instruction.opcode == Opcode::DictionaryPlace)
                    {
                        ...
                        {
                            report("LIR1445",
                                   "Lowered WIR dictionary place requires a dictionary reference, matching key, and "
                                   "mapped reference.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::Borrow)
                    {
                        ...
                        {
                            report("LIR1430",
                                   "Lowered WIR borrow cannot change the referred type or strengthen mutability.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                             ...
                             instruction.opcode == Opcode::ConstructObject)
                    {
                        ...
                        {
                            report("LIR1433",
                                   "Lowered WIR construction requires a matching constructible component/object result "
                    ...
                    else if (instruction.opcode == Opcode::Retain || instruction.opcode == Opcode::CopyValue)
                    {
                        ...
                        {
                            report("LIR1446", "Lowered WIR retain/copy-value must match the type cleanup protocol.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::MoveValue)
                    {
                        ...
                        {
                            report("LIR1447", "Lowered WIR move-value must transfer one cleanup-bearing place.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::Release || instruction.opcode == Opcode::DropValue)
                    {
                        ...
                        {
                            report("LIR1448", "Lowered WIR release/drop-value must match the value cleanup protocol.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::ReleasePlace || instruction.opcode == Opcode::DropPlace)
                    {
                        ...
                        {
                            report("LIR1434",
                                   "Lowered WIR release/drop-place must match the stored value cleanup protocol.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    ...
                    else if (instruction.opcode == Opcode::IndirectCall)
                    {
                        ...
                        if (!valid)
                            report("LIR1441", "Lowered WIR indirect call must match its function value signature.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::ExtensionCall)
                    {
                        ...
                        if (valid)
                        {
                            ...
                            valid = valid &&
                                    (valueType(instruction.operands.front()) == instruction.signatureTypes.front() ||
                        ...
                        if (!valid)
                            report("LIR1442",
                                   "Lowered WIR extension call must pin its implementation and concrete signature.",
                                   instruction.source, function.id, block.id);
                    }
                             ...
                             instruction.opcode == Opcode::InterfaceCall)
                    {
                        ...
                        if (!valid)
                            report("LIR1435",
                                   "Lowered WIR method dispatch must match its owner slot, receiver, signature, and "
                                   "callee.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::Upcast || instruction.opcode == Opcode::CheckedCast)
                    {
                        ...
                        if (!valid)
                            report(
                                "LIR1436",
                                "Lowered WIR object cast requires compatible object/interface source and target types.",
                                instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::TypeTest)
                    {
                        if (instruction.operands.size() != 1 ||
                            ...
                            instruction.resultType != module.types.boolType())
                            report("LIR1437",
                                   "Lowered WIR type test requires an object/interface value, target type, and bool "
                                   "result.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::IdentityEqual)
                    {
                        if (instruction.operands.size() != 2 ||
                            ...
                            report("LIR1438",
                                   "Lowered WIR identity equality requires object/interface operands and bool result.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::Call || instruction.opcode == Opcode::NativeInvoke)
                    {
                        ...
                        else
                        {
                            ...
                            if (!callArityMatches(module.types, callee, instruction.operands.size()) ||
                                !concreteSignature)
                                report("LIR1408", "Lowered WIR call argument count does not match its callee.",
                                       instruction.source, function.id, block.id);
                            else
                            {
                                     ...
                                     ++argumentIndex)
                                {
                                    ...
                                    const Type* signatureType =
                                        module.types.tryGet(instruction.signatureTypes[argumentIndex]);
                                    const bool extensionReceiverMatch = callee.isExtension && argumentIndex == 0 &&
                                                                        signatureType &&
                                    ...
                                    if ((!extensionReceiverMatch &&
                                        ...
                                        (callee.genericParameters.empty() &&
                                         instruction.signatureTypes[argumentIndex] !=
                                             callee.parameters[argumentIndex].type))
                                        report("LIR1409",
                                               "Lowered WIR call argument type does not match its parameter.",
                                               instruction.source, function.id, block.id);
                                }
                            }
                            ...
                            if (!returnType || (returnsVoid ? instruction.result.isValid() : !resultMatches))
                                report("LIR1410", "Lowered WIR call result does not match its callee.",
                                       instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::CancellationCheck)
                    {
                        ...
                        if (!function.isAsync || !instruction.operands.empty() || instruction.result ||
                            !followedBySuspend)
                            report("LIR1454",
                                   "Cancellation check must immediately precede its coroutine suspension point.",
                                   instruction.source, function.id, block.id);
                    }
                    else if (instruction.opcode == Opcode::CoroutineSuspend)
                    {
                        const CoroutineState* state =
                            function.coroutine && instruction.projectionIndex < function.coroutine->states.size()
                                ? &function.coroutine->states[instruction.projectionIndex]
                                : nullptr;
                        const bool operationShape =
                                      ...
                                      [&]
                        {
                            ...
                            return task && task->kind == TypeKind::AsyncTask && task->arguments.size() == 1 && state &&
                                   state->resultType == task->arguments.front();
                        }()
                                : instruction.asyncOperation == AsyncOperation::SwitchExecutor &&
                                      instruction.operands.empty() &&
                                      (instruction.asyncExecutor == AsyncExecutorKind::Main ||
                        ...
                        const bool precededByCancellationCheck =
                        ...
                        if (!function.isAsync || !state || block.id != state->suspendBlock ||
                            ...
                            (state && (state->executor != instruction.asyncExecutor ||
                                       state->awaitedTask !=
                                           (instruction.operands.empty() ? ValueId{} : instruction.operands.front()) ||
                                       !state->cancellationPoint)) ||
                            !operationShape || !precededByCancellationCheck)
                        {
                            report("LIR1455", "Coroutine suspension must match one canonical state and resume target.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::CoroutineResume)
                    {
                        ...
                        {
                            report("LIR1456",
                                   "Coroutine resume must materialize the payload declared by its canonical state.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::CoroutineComplete)
                    {
                        ...
                        {
                            report("LIR1457", "Coroutine completion value must match the async payload type.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    ...
                    {
                        if (function.isAsync)
                            report("LIR1458", "Async Lowered WIR must use coroutine-complete instead of return.",
                                   instruction.source, function.id, block.id);
                        ...
                        if (!returnType ||
                            (returnsVoid ? !instruction.operands.empty()
                                         : instruction.operands.size() != 1 ||
                                               valueType(instruction.operands.front()) != function.returnType))
                        {
                            report("LIR1411", "Lowered WIR return value does not match its function.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    else if (instruction.opcode == Opcode::Jump || instruction.opcode == Opcode::CondJump)
                    {
                        ...
                        if (instruction.targets.size() != expectedTargets ||
                            instruction.operands.size() != expectedOperands)
                        {
                            report("LIR1412", "Lowered WIR jump shape is invalid.", instruction.source, function.id,
                                   block.id);
                        }
                        ...
                        {
                            report("LIR1413", "Lowered WIR conditional jump condition must be bool.",
                                   instruction.source, function.id, block.id);
                        }
                    }
                    ...
                    for (const BranchTarget& target : instruction.targets)
                    {
                        ...
                        {
                            if (valueType(target.arguments[argumentIndex]) !=
                                targetBlock.parameters[argumentIndex].type)
                                report("LIR1416",
                                       "Lowered WIR jump argument type does not match target block parameter.",
                                       instruction.source, function.id, block.id);
                        }
                    }
                }
