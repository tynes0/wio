...
void CppGenerator::visit(VariableDeclaration& node)
{
    ...
    if (resolvedVarType && resolvedVarType->kind() == sema::TypeKind::Array && node.initializer)
    {
        auto arrayType = resolvedVarType.AsFast<sema::ArrayType>();
        if (arrayType->arrayKind == sema::ArrayType::ArrayKind::Static && arrayType->size == 0)
        {
            if (const auto* functionCall = node.initializer->as<FunctionCallExpression>())
            {
                if (const auto* memberAccess =
                        functionCall->callee ? functionCall->callee->as<MemberAccessExpression>() : nullptr;
    ...
    if (node.mutability == Mutability::Const)
    {
        ...
        const bool requiresRuntimeConstStorage =
            ...
            (resolvedVarType && resolvedVarType->kind() == sema::TypeKind::Primitive &&
...
void CppGenerator::visit(FunctionDeclaration& node)
{
    ...
    if (!isEmittingPrototypes_ && node.body && !isNative)
    {
        ...
        std::ranges::stable_sort(orderedAttributes, {}, &AttributeStatement::processorOrder);
    ...
    if (funcName == "Entry" && !node.isAsync && node.genericParameters.empty() &&
        Compiler::get().getBuildTarget() == BuildTarget::Executable && (!sym || sym->scopePath.empty()))
    {
    ...
    {
        auto bindings =
            buildExtendedGenericBindings(sym->genericParameterNames, sym->hasGenericParameterPack, instantiationTypes);
    ...
    auto emitWrapperParameters = [&](size_t providedFixedParameterCount)
    {
        ...
        {
            emit(common::formatString("{} {}", toCppType(funcType->paramTypes[i]),
        ...
        if (funcType->hasParameterPack && !node.parameters.empty())
        {
            ...
            emit(common::formatString("{} {}...", toCppType(funcType->paramTypes.back()),
            ...
            if (!node.genericParameters.empty())
            {
                ...
                for (size_t i = 0; i < node.genericParameters.size(); ++i)
                {
                    const bool isGenericParameterPack =
                        node.hasGenericParameterPack && i + 1 == node.genericParameters.size();
                    emit(formatCppTemplateParameter(node.genericParameters[i], isGenericParameterPack));
                    if (i + 1 < node.genericParameters.size())
                        emit(", ");
                }
    ...
    if (funcName != "OnDestruct")
    {
        for (size_t i = 0; i < node.parameters.size(); ++i)
        {
            ...
            if (isEmittingPrototypes_ && funcType && funcType->hasParameterPack && param.defaultValue &&
        ...
        if (!currentClassName_.empty())
        {
            if (hasAttribute(node.attributes, Attribute::Final))
                emit(" final");
        }
    }
    ...
    if (isNative)
    {
        ...
        auto buildNativeReferencePreferredExpr = [&](const std::string& expr,
                                                     const Ref<sema::Type>& type) -> std::string
        {
            auto resolvedType = unwrapAliasTypeForCodegen(type);
            if (!resolvedType || resolvedType->kind() != sema::TypeKind::Reference)
                return expr;
        ...
        buildWioToNativePodExpr = [&](const std::string& expr, const Ref<sema::Type>& sourceType,
                                      bool sourceIsPointer) -> std::string
        {
        ...
        emitNativePodCopyBack = [&](const std::string& destinationExpr, const Ref<sema::Type>& destinationType,
                                    const std::string& sourceExpr, bool destinationIsPointer)
        {
            ...
            for (size_t fieldIndex = 0; fieldIndex < nativeStruct->fieldNames.size(); ++fieldIndex)
            {
                ...
                const std::string destinationFieldExpr =
                    destinationExpr + (destinationIsPointer ? "->" : ".") + wioFieldName;
                const std::string sourceFieldExpr = sourceExpr + "." + nativeFieldName;
                auto nestedNativeStruct =
                    getNativePodComponentStructTypeForCodegen(nativeStruct->fieldTypes[fieldIndex]);
                if (nestedNativeStruct)
                {
                    emitNativePodCopyBack(destinationFieldExpr, nativeStruct->fieldTypes[fieldIndex], sourceFieldExpr,
                                          false);
                }
                else
                {
        ...
        for (size_t i = 0; i < node.parameters.size(); ++i)
        {
            ...
            const bool usesNativePodAliasModel = nativeStruct && usesNativePodAliasModelForCodegen(nativeStruct);
            ...
            if (!nativeStruct)
            {
                if (isDirectStringNativeInteropType(parameterType))
                {
                    preparedArguments.push_back(
                        {common::formatString("wio::intrinsics::NativeStringArg({})", parameterName),
                         common::formatString("wio::intrinsics::NativeStringArg({})", parameterName),
                         common::formatString("wio::intrinsics::NativeStringArg({})", parameterName), "", nullptr,
                         false, false, toCppType(parameterType), toCppType(parameterType), toCppType(parameterType)});
                    continue;
                }
                ...
                if (shouldUseNativeReferenceWrapper(parameterType))
                {
                    ...
                    preparedArguments.push_back({parameterName,
                                                 buildNativeReferencePreferredExpr(parameterName, parameterType),
                                                 parameterName, "", nullptr, false, true, fallbackSignatureType,
                                                 preferredSignatureType, fallbackSignatureType});
                    usesNativeReferenceWrappers = true;
                    continue;
                }

                preparedArguments.push_back({parameterName + (node.parameters[i].isParameterPack ? "..." : ""),
                                             parameterName + (node.parameters[i].isParameterPack ? "..." : ""),
                                             parameterName + (node.parameters[i].isParameterPack ? "..." : ""), "",
                                             nullptr, false, false, toCppType(parameterType), toCppType(parameterType),
            ...
            if (usesNativePodAliasModel)
            {
                ...
                const bool usesExtensionReceiverDispatch = isReferenceParameter && node.isExtensionMethod && i == 0;
                ...
                preparedArgument.callExpr =
                    isReferenceParameter && !usesExtensionReceiverDispatch ? "*" + parameterName : parameterName;
                preparedArgument.preferredCallExpr = isReferenceParameter ? "*" + parameterName : parameterName;
                preparedArgument.fallbackCallExpr =
                    usesExtensionReceiverDispatch ? parameterName : preparedArgument.preferredCallExpr;
                ...
                preparedArgument.fallbackSignatureType =
                    usesExtensionReceiverDispatch
                        ? (isMutableReference ? nativeSignatureType + "*" : "const " + nativeSignatureType + "*")
                        : preparedArgument.signatureType;
            ...
            EMIT_TABS();
            emitLine(getNativePodCppName(nativeStruct) + " " + nativeTempName + " = " +
                     buildWioToNativePodExpr(parameterName, parameterType,
                                             resolvedParameterType &&
                                                 resolvedParameterType->kind() == sema::TypeKind::Reference) +
                     ";");
        ...
        const bool emitExplicitNativeTemplateArguments = !node.genericParameters.empty() && [&]() -> bool
        {
            ...
            return cppNameArg->value == "wio::runtime::EnumCount" || cppNameArg->value == "wio::runtime::EnumName" ||
                   cppNameArg->value == "wio::runtime::EnumValue" || cppNameArg->value == "wio::runtime::EnumIndex" ||
                   cppNameArg->value == "wio::runtime::EnumUnderlyingTypeName" ||
                   cppNameArg->value == "wio::runtime::EnumSize" || cppNameArg->value == "wio::runtime::EnumIsValid" ||
        ...
        if (usesNativeReferenceWrappers)
        {
            ...
            for (size_t argumentIndex = 0; argumentIndex < preparedArguments.size(); ++argumentIndex)
            {
                ...
                {
                    emit(common::formatString("std::forward<decltype(_wio_native_arg{})>(_wio_native_arg{})",
            ...
            for (size_t argumentIndex = 0; argumentIndex < preparedArguments.size(); ++argumentIndex)
            {
                if (argumentIndex > 0)
                    emit(", ");
                emit(common::formatString("std::forward<decltype(_wio_native_arg{})>(_wio_native_arg{})", argumentIndex,
                                          argumentIndex));
            }
            emit(")");
            emit(";");
        ...
        for (const auto& preparedArgument : preparedArguments)
        {
            ...
            emitNativePodCopyBack(preparedArgument.mutableTargetName, preparedArgument.mutableTargetType,
                                  preparedArgument.callExpr, preparedArgument.mutableTargetIsPointer);
        }
        ...
        emitLine("{");
        indent();
        emitLine("throw wio::runtime::RuntimeException(\"Native call '" +
                 common::wioStringToEscapedCppString(nativeSymbol) + "' failed: \" + std::string(ex.what()));");
        dedent();
        emitLine("}");
        emitLine("catch (...)");
        emitLine("{");
        indent();
        emitLine("throw wio::runtime::RuntimeException(\"Native call '" +
                 common::wioStringToEscapedCppString(nativeSymbol) + "' failed with an unknown exception.\");");
        dedent();
        emitLine("}");
        dedent();
        emitLine("}");
    ...
    else if (node.body)
    {
        auto emitFunctionBody = [&]()
        {
            ...
            {
                emitLine("auto " + processor.variableName + " = wio::runtime::Ref<" + processor.cppTypeName +
                         ">::Create();");
                if (processor.phase == "finally")
                    emitLine("bool " + processor.finalizedFlagName + " = false;");
            }

            const bool hasFinallyProcessor =
                std::ranges::any_of(behavioralProcessors, [](const BehavioralProcessorInstance& processor)
                                    { return processor.phase == "finally"; });
            const bool hasAroundProcessor =
                std::ranges::any_of(behavioralProcessors, [](const BehavioralProcessorInstance& processor)
            ...
            for (const auto& processor : behavioralProcessors)
            {
                if (processor.phase == "pre")
                {
                    ...
                    {
                        arguments = "wio::runtime::Any::FromObject<" + currentClassName_ + ">(wio::runtime::Ref<" +
                                    currentClassName_ + ">(this))";
                    }
                    ...
                    const std::string invocation =
                        processor.variableName + "->" + processor.hookCppName + "(" + arguments + ")";
                    if (processor.hookMode.ends_with("_guard"))
                        emitLine("if (!(" + invocation + ")) " + std::string(node.isAsync ? "co_return;" : "return;"));
                    else
                        emitLine(invocation + ";");
                }
            }
            ...
            for (auto processor = behavioralProcessors.rbegin(); processor != behavioralProcessors.rend(); ++processor)
            {
                ...
                else if (processor->phase == "finally")
                {
                    currentBehavioralFinallyProcessors_.push_back(
                        "if (!" + processor->finalizedFlagName + ") { " + processor->finalizedFlagName + " = true; " +
                        processor->variableName + "->" + processor->hookCppName +
            ...
            if (hasFinallyProcessor)
            {
                ...
                emitLine("{");
                indent();
                for (auto processor = behavioralProcessors.rbegin(); processor != behavioralProcessors.rend();
                     ++processor)
                {
                    if (processor->phase == "finally")
                    {
            ...
            if (hasAroundProcessor)
            {
                     ...
                     ++processor)
                {
                    ...
                    const std::string guardName = "_wio_attribute_proceed_guard_" + std::to_string(aroundIndex);
                    const bool returnsResult = processor->hookMode == "proceed_result";
                    const std::string aroundResultType =
                        returnsResult ? toCppType(currentFunctionReturnType_) : std::string("void");
                    emitLine("auto " + wrapperName + " = [&]()" + (returnsResult ? " -> " + aroundResultType : ""));
                    emitLine("{");
                    indent();
                    emitLine("auto " + stateName + " = std::make_shared<std::pair<bool, bool>>(true, false);");
                    emitLine("auto " + guardName + " = std::shared_ptr<void>(nullptr, [" + stateName + "](void*) { " +
                             stateName + "->first = false; });");
                    emitLine("try");
                    emitLine("{");
                    indent();
                    emitLine(std::string(returnsResult ? "return " : "") + processor->variableName + "->" +
                             processor->hookCppName + "(std::function<" + aroundResultType + "()>([&, " + stateName +
                             "]()" + (returnsResult ? " -> " + aroundResultType : ""));
                    emitLine("{");
                    indent();
                    emitLine("if (!" + stateName +
                             "->first) throw wio::runtime::RuntimeException(\"Attribute Proceed escaped its Around "
                             "invocation.\");");
                    emitLine("if (" + stateName +
                             "->second) throw wio::runtime::RuntimeException(\"Attribute Proceed may be invoked at "
                             "most once.\");");
                ...
                emitLine(
                    std::string(currentFunctionReturnType_ && !currentFunctionReturnType_->isVoid() ? "return " : "") +
                    nextProceed + "();");
            }
            dedent();
            emitLine("}");
        ...
        const bool catchesResultPropagation = [&]()
        {
            ...
            return structType && (structType->name == "Result" || structType->name == "ResultValue") &&
                   structType->scopePath == "std" && structType->genericArguments.size() == 1;
        }();
        ...
        if (catchesResultPropagation)
        {
            ...
            emitLine("catch (const decltype(std::declval<" + propagationReturnType +
                     ">()->_WF_ErrorValue())& _wio_result_error)");
            emitLine("{");
            indent();
            emitLine(std::string(node.isAsync ? "co_return " : "return ") + propagationReturnType +
                     "::Create(_wio_result_error);");
            dedent();
            emitLine("}");
        }
    ...
    if (!isEmittingPrototypes_ && !currentClassName_.empty() && sym && !sym->overriddenSymbols.empty() &&
        node.genericParameters.empty())
    {
        ...
        for (const auto& overriddenWeak : sym->overriddenSymbols)
        {
            ...
            auto overriddenType =
                overridden && overridden->type ? overridden->type.AsFast<sema::FunctionType>() : nullptr;
            if (!overriddenType)
                continue;
            ...
            for (size_t parameterIndex = 0; parameterIndex < node.parameters.size(); ++parameterIndex)
            {
                ...
                emit(common::formatString("{} {}", toCppType(parameter.name->refType.Lock()),
    ...
    if (emitsExportWrapper && !isEmittingPrototypes_ && currentClassName_.empty() && node.body)
    {
        ...
        if (!node.genericParameters.empty() && !instantiationTypeLists.empty())
        {
            ...
            auto getExportWrapperParameterName = [&](size_t parameterIndex) -> std::string
            {
                ...
                const std::string packBaseName = sanitizeCppIdentifier(node.parameters.back().name->token.value);
            ...
            for (const auto& instantiationTypes : instantiationTypeLists)
            {
                ...
                for (size_t i = 0; i < instantiatedFunctionType->paramTypes.size(); ++i)
                {
                    emit(common::formatString("{} {}", toCppType(instantiatedFunctionType->paramTypes[i]),
        ...
        else
        {
            ...
            {
                auto& param = node.parameters[i];
                emit(common::formatString("{} {}", toCppType(param.name->refType.Lock()),
                                          sanitizeCppIdentifier(param.name->token.value)));
                if (i < node.parameters.size() - 1)
                    emit(", ");
            }
            emit(")");
    ...
    if (!isEmittingPrototypes_ && currentClassName_.empty() && !node.genericParameters.empty() &&
        !instantiationTypeLists.empty() && (isNative || isExported))
    {
        emitGeneratedDirective();
        emitLine();
    ...
    if (node.isAsync && funcName == "Entry" && !isEmittingPrototypes_ && node.genericParameters.empty() &&
        Compiler::get().getBuildTarget() == BuildTarget::Executable && (!sym || sym->scopePath.empty()))
    {
        emitMain(node);
    }
...
void CppGenerator::visit(InterfaceDeclaration& node)
{
    ...
    for (auto& method : node.methods)
    {
        ...
        {
            emit(common::formatString("{} {}", toCppType(method->parameters[i].name->refType.Lock()),
                                      sanitizeCppIdentifier(method->parameters[i].name->token.value)));
...
void CppGenerator::visit(ComponentDeclaration& node)
{
    ...
    auto enclosingScope =
        componentSym && componentSym->innerScope ? componentSym->innerScope->getParent().Lock() : nullptr;

    if (componentType && componentType->isExplicitSpecialization && usesNativePodAliasModelForCodegen(componentType))
    {
        return;
    }
    ...
    const std::string declaredClassName = Mangler::mangleStruct(
        componentType ? componentType->name : node.name->token.value, componentType ? componentType->scopePath : "");
    if (componentType && usesNativePodAliasModelForCodegen(componentType))
    {
        std::string nativeTypeName =
            componentType->nativeCppName.empty() ? componentType->name : componentType->nativeCppName;
        if (!node.genericParameters.empty())
        {
            nativeTypeName += "<";
            for (size_t i = 0; i < node.genericParameters.size(); ++i)
        ...
        if (member.declaration->is<FunctionDeclaration>())
        {
            ...
            if (funcDecl->name->token.value == "OnConstruct")
            {
                ...
                else if (pCount == 1)
                {
                    ...
                    if (pType.find(currentClassName_) != std::string::npos)
                        hasCopyCtor = true;
                }
...
void CppGenerator::visit(ObjectDeclaration& node)
{
    ...
    const std::string declaredClassName = Mangler::mangleStruct(objectType ? objectType->name : node.name->token.value,
    ...
    for (const auto& base : bases)
    {
        ...
        {
            emitLine(common::formatString("if (void* base_cast = {}::_WF_CastTo(id)) return base_cast;",
                                          mangleNamedType(base)));
        }
    }
    ...
    for (auto& member : node.members)
    {
        ...
        {
            dedent();
            if (targetAccess == AccessModifier::Public)
                emitLine("public:");
            else if (targetAccess == AccessModifier::Private)
                emitLine("private:");
            else if (targetAccess == AccessModifier::Protected)
                emitLine("protected:");
            indent();
...
void CppGenerator::visit(FlagsetDeclaration& node)
{
    ...
    {
        std::string nativeTypeName =
            flagsetType->nativeCppName.empty() ? flagsetType->name : flagsetType->nativeCppName;
        EMIT_TABS();
    ...
    emitLine(common::formatString("inline constexpr {0} operator|({0} a, {0} b) {{ return "
                                  "static_cast<{0}>(static_cast<{1}>(a) | static_cast<{1}>(b)); }}",
                                  enumName, underType));
    emitLine(common::formatString("inline constexpr {0} operator&({0} a, {0} b) {{ return "
                                  "static_cast<{0}>(static_cast<{1}>(a) & static_cast<{1}>(b)); }}",
                                  enumName, underType));
    ...
    emitLine(common::formatString(
        "inline constexpr {0} operator~({0} a) {{ return static_cast<{0}>(~static_cast<{1}>(a)); }}", enumName,
        underType));
}
