    ...
    std::string WirReflectionEmitter::traits(const wir::lowered::Module& m, const WirModuleEmitter::TypeSpelling& cpp)
    {
        ...
        std::function<bool(TypeId)> concrete = [&](TypeId t)
        {
            ...
            if (v.kind == TypeKind::GenericParameter || v.kind == TypeKind::ConstGenericParameter ||
                v.kind == TypeKind::GenericParameterPack || v.kind == TypeKind::TypePack ||
                v.kind == TypeKind::ValuePack)
                return false;
        ...
        for (const auto& r : m.contract.reflection)
        {
            ...
            out << "template <> struct wio::runtime::TypeReflection<" << cpp(r.type)
                ...
                << kind << ";\n";
            std::vector<std::string> names, types, access, methods, signatures, methodAccess, bases, typeAttrs,
                attrNames, attrRetentions, attrOrigins, attrArgNames, attrArgTypes, attrArgValues, fieldAttrs,
                behaviorAttrs, behaviorTypes, behaviorPhases, behaviorHooks, behaviorModes;
            ...
            out << "};\n";
            out << "    static constexpr std::array<std::size_t, " << behaviorOffsets.size()
                << "> MethodBehaviorOffsets{";
                                           ...
                                           const ThunkEmitter& thunk)
    {
        ...
        for (const auto& r : m.contract.reflection)
        {
            ...
            if (!r.fields.empty())
            {
                ...
                {
                    const auto& f = r.fields[i];
                    out << "    {" << f.stableId << "ULL, " << WirCppText::quote(f.name) << ", " << id(f.type)
                        << "ULL, " << static_cast<int>(f.visibility) << ", " << f.isMutable << ", " << getters[i]
                        << ", " << setters[i] << "},\n";
                }
                out << "};\n";
            }
        ...
        if (!attributes.empty())
        {
            ...
            {
                auto p = "_attr_" + std::to_string(a->stableId);
                out << "    {" << a->stableId << "ULL, " << WirCppText::quote(a->canonicalName) << ", "
                    << a->targetStableId << "ULL, " << static_cast<int>(a->targetKind) << ", "
                    << static_cast<int>(a->origin) << ", " << WirCppText::quote(a->originParent) << ", "
                    << WirCppText::quote(a->selector) << ", " << a->parameterIndex << ", " << a->sourceOrder << ", "
                    << a->arguments.size() << ", " << (a->arguments.empty() ? "nullptr" : p + "_args") << ", "
                    << a->processors.size() << ", " << (a->processors.empty() ? "nullptr" : p + "_processors")
                    << "},\n";
            }
            out << "};\n";
        }
