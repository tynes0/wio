...
void SemanticAnalyzer::visit(ArrayAccessExpression& node)
{
    ...
    auto tryResolveIndexOperatorOverload = [&]() -> bool
    {
        ...
        auto deduceBindingsFromIndexArgument = [&](const Ref<Type>& expectedType, const Ref<Type>& argumentType,
        ...
        auto scoreIndexArgumentAgainstParameter =
            [&](const Ref<Type>& parameterType, const Ref<Type>& rawArgumentType, const Ref<Type>& readableArgumentType,
                const NodePtr<Expression>& argumentExpression) -> std::optional<int>
        {
            ...
            if (resolvedParameterType && resolvedParameterType->kind() == TypeKind::Reference)
            {
                ...
                if (readableArgumentType &&
                    isAssignmentLikeCompatible(referenceType->referredType, readableArgumentType))
        ...
        auto appendMemberCandidates = [&](const Ref<Type>& candidateReceiverType)
        {
            ...
            Ref<Symbol> candidateSymbol =
                findStructMemberInHierarchy(currentType, std::string(*overloadName), &ownerType);
            if (!candidateSymbol)
                return;
            ...
            for (const auto& overload : overloads)
            {
                ...
                {
                    if (!deduceBindingsFromIndexArgument(candidateFunctionType->paramTypes[0], idxType, node.index,
                                                         genericBindings))
                        continue;
                ...
                auto score = scoreIndexArgumentAgainstParameter(candidateFunctionType->paramTypes[0], idxType,
                                                                getAutoReadableType(idxType), node.index);
                if (!score.has_value())
                    continue;

                candidates.push_back(OperatorCandidate{.symbol = overload,
        ...
        auto appendFreeSymbol = [&](const Ref<Symbol>& candidateSymbol)
        {
            ...
            for (const auto& overload : overloads)
            {
                ...
                if (!overload->genericParameterNames.empty())
                {
                    if (!deduceBindingsFromIndexArgument(candidateFunctionType->paramTypes[0], objType, node.object,
                                                         genericBindings) ||
                        !deduceBindingsFromIndexArgument(candidateFunctionType->paramTypes[1], idxType, node.index,
                                                         genericBindings))
                    {
                        continue;
                    }
                ...
                auto objectScore = scoreIndexArgumentAgainstParameter(candidateFunctionType->paramTypes[0], objType,
                                                                      getAutoReadableType(objType), node.object);
                if (!objectScore.has_value())
                    continue;

                auto indexScore = scoreIndexArgumentAgainstParameter(candidateFunctionType->paramTypes[1], idxType,
                                                                     getAutoReadableType(idxType), node.index);
        ...
        if (isAmbiguous)
        {
            WIO_LOG_ADD_ERROR(node.location(), "Ambiguous overload for operator '[]' with operand types '{}' and '{}'.",
                              objType ? objType->toString() : "<unknown>", idxType ? idxType->toString() : "<unknown>");
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return true;
        }
        ...
        node.refType = bestCandidate->functionType ? bestCandidate->functionType->returnType
        ...
        if (!elementTypes.empty())
        {
            if (elementTypes.size() == 1)
            {
                if (auto symbolicPackName = tryGetSymbolicPackReferenceName(elementTypes.front()))
                {
                    ...
                    node.packElementBindingName = makePackElementBindingName(reboundBinding);
            ...
            {
                node.resolvedPackIndex = *resolvedIndex;
                ParsedPackElementBinding resolvedBinding = *indexBinding;
                resolvedBinding.packName = packName;
                node.packElementBindingName = makePackElementBindingName(resolvedBinding);
                return elementTypes[*resolvedIndex];
            }

            WIO_LOG_ADD_ERROR(node.index->location(), "Pack index is out of range for size {}.", elementTypes.size());
            return Compiler::get().getTypeContext().getUnknown();
        }
    ...
    {
        node.refType = resolvePackElementType(resolvedObjType.AsFast<GenericParameterPackType>()->name, {});
        return;
    }
    ...
    if (resolvedObjType->kind() != TypeKind::Array && resolvedObjType->kind() != TypeKind::Dictionary &&
        !isStringType(resolvedObjType) && !isTextType(resolvedObjType))
    {
        WIO_LOG_ADD_ERROR(node.object->location(),
                          "Type '{}' is not an array, dictionary, string, or text and cannot be indexed.",
                          objType->toString());
        node.refType = Compiler::get().getTypeContext().getUnknown();
        return;
    }
    ...
    if (resolvedObjType->kind() == TypeKind::Array)
    {
        ...
        if (auto staticIndex = tryEvaluateStaticPackIndex(node.index, variableDeclarationsBySymbol_);
            ...
            (arrType->arrayKind == ArrayType::ArrayKind::Static ||
             arrType->arrayKind == ArrayType::ArrayKind::Literal) &&
        ...
        {
            WIO_LOG_ADD_ERROR(node.index->location(), "Index {} is out of range for compile-time array size {}.",
                              *staticIndex, arrType->size);
        }
    ...
    node.refType = isTextType(resolvedObjType) ? Compiler::get().getTypeContext().getText()
...
void SemanticAnalyzer::visit(MemberAccessExpression& node)
{
    ...
    if (resolvedLeftType &&
        (resolvedLeftType->kind() == TypeKind::GenericParameterPack ||
         resolvedLeftType->kind() == TypeKind::ValuePackView || resolvedLeftType->kind() == TypeKind::TypePackView ||
         resolvedLeftType->kind() == TypeKind::PackStorage))
    {
        ...
        {
            auto normalizePackName = [&](const std::string& fallbackName,
                                         const std::vector<Ref<Type>>& elementTypes) -> std::string
            {
                if (elementTypes.size() == 1)
                {
                    if (auto symbolicPackName = tryGetSymbolicPackReferenceName(elementTypes.front()))
        ...
        if (memberName == "array")
        {
            ...
            const bool isValuePackReference =
                ...
                resolvedLeftType->kind() == TypeKind::PackStorage ||
                (resolvedLeftType->kind() == TypeKind::GenericParameterPack && leftSymbol &&
    ...
    {
        auto overloads =
            resolveIntrinsicMemberOverloads(Compiler::get().getTypeContext(), candidateType, node.member->token.value);
        if (!overloads.empty())
        {
            if (overloads.size() == 1)
            {
                ...
                if (node.member->token.value == "Get" &&
                    (resolution.member == IntrinsicMember::ArrayGet || resolution.member == IntrinsicMember::DictGet ||
                     resolution.member == IntrinsicMember::StringGet))
                {
                    ...
                    auto optionStruct =
                        optionSymbol && optionSymbol->type && optionSymbol->type->kind() == TypeKind::Struct
                    ...
                    Ref<Type> optionType =
                        instantiateGenericStructType(optionStruct, {optionPayloadType}, node.location());
                    resolution.memberType = Compiler::get().getTypeContext().getOrCreateFunctionType(
                        optionType, resolution.memberType.AsFast<FunctionType>()->paramTypes);
                }
                ...
                else if (resolution.member == IntrinsicMember::TaskWithin)
                {
                    ...
                    else if (taskType)
                    {
                        ...
                        {
                            Ref<Type> optionType =
                                instantiateGenericStructType(optionStruct, {taskType->valueType}, node.location());
                ...
                {
                    WIO_LOG_ADD_ERROR(node.location(), "Container member '{}' requires a mutable receiver.",
    ...
    if (leftSymbol && leftSymbol->kind == SymbolKind::Namespace)
    {
        ...
        {
            WIO_LOG_ADD_ERROR(node.location(), "The namespace contents are inaccessible. (No Scope): {}",
                              leftSymbol->name);
            return;
        }
    ...
    else
    {
        ...
        {
            WIO_LOG_ADD_ERROR(node.member->location(),
                              "Cannot access member '{}'. The left-hand side has no resolved type.",
                              node.member->token.value);
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        }
        ...
        else if (baseType->kind() == TypeKind::Reference)
        {
            ...
            {
                WIO_LOG_ADD_ERROR(node.member->location(),
                                  "Cannot access member '{}'. Reference points to '{}', which is not a struct, array, "
                                  "dictionary, or string.",
                                  node.member->token.value, referredType ? referredType->toString() : "Unknown");
                node.refType = Compiler::get().getTypeContext().getUnknown();
                return;
            }
        }
    ...
    if (!foundMember && actualStructType)
    {
        ...
        if (Ref<Symbol> extensionSymbol = findExtensionMethod(actualStructType))
        {
            ...
            {
                std::vector<Ref<Type>> visibleParameters(fullType->paramTypes.begin() + 1, fullType->paramTypes.end());
                ...
                auto receiverReference = receiverType && receiverType->kind() == TypeKind::Reference
                                             ...
                                             : nullptr;
                if (receiverReference && receiverReference->isMutable && !canMutateIntrinsicReceiver(node.object))
                {
                    WIO_LOG_ADD_ERROR(node.location(), "Extension method '{}' requires a mutable receiver.",
                                      node.member->token.value);
                }

                Ref<Symbol> callableSymbol = createSymbol(extensionSymbol->name, visibleType, SymbolKind::Function,
    ...
    if (!foundMember)
    {
        ...
        WIO_LOG_ADD_ERROR(node.member->location(), "Member not found: '{}' in '{}'", node.member->token.value,
                          ownerName);
        node.refType = Compiler::get().getTypeContext().getUnknown();
        return;
    }
    ...
    {
        if (currentStructType_ == actualStructType || isTypeDerivedFrom(currentStructType_, actualStructType) ||
        ...
        if (auto ownerStruct = actualStructType.AsFast<StructType>(); ownerStruct)
        {
            ...
            isTrustedAccess = !trustedKey.empty() && std::ranges::find(ownerStruct->trustedTypeKeys, trustedKey) !=
                                                         ownerStruct->trustedTypeKeys.end();
        }
    }
    ...
    {
        WIO_LOG_ADD_ERROR(node.location(), "Cannot access private member '{}' declared on '{}' from '{}'.",
                          foundMember->name, ownerTypeName, currentContextTypeName);
    }

    if (foundMember->flags.get_isProtected() && !isInsideHierarchy && !isTrustedAccess)
    {
        WIO_LOG_ADD_ERROR(node.location(), "Cannot access protected member '{}' declared on '{}' from '{}'.",
                          foundMember->name, ownerTypeName, currentContextTypeName);
    }
    ...
    if (auto instantiatedStructType = actualStructType ? actualStructType.AsFast<StructType>() : nullptr;
        instantiatedStructType && !instantiatedStructType->genericParameterNames.empty() &&
        !instantiatedStructType->genericArguments.empty())
    {
        auto bindings = buildExtendedGenericBindings(instantiatedStructType->genericParameterNames,
...
{
    auto formatAppliedTypeName = [](const std::string& baseName,
                                    const std::vector<Ref<Type>>& typeArguments) -> std::string
    {
    ...
    auto satisfiesApplyForSymbol = [&](const Ref<Symbol>& symbol, const std::vector<Ref<Type>>& explicitTypeArguments,
                                       std::string_view declarationKind) -> bool
    {
        ...
        if (!matchesApplyConstraints(*attributeIt->second, symbol->genericParameterNames,
                                     symbol->hasGenericParameterPack, explicitTypeArguments))
        {
            WIO_LOG_ADD_ERROR(
                node.location(), "Generic {} '{}' rejects type arguments {} because of @Apply constraints.",
                declarationKind, symbol->name, formatConcreteInstantiationSignature(explicitTypeArguments));
            return false;
        }
    ...
    if (calleeSym && calleeSym->kind == SymbolKind::Function && !explicitTypeArguments.empty() &&
        !calleeSym->genericParameterNames.empty() &&
        !validateGenericArgumentKinds(calleeSym->genericParameterNames, calleeSym->genericParameterTypes,
                                      explicitTypeArguments, node.location()))
    {
        node.refType = Compiler::get().getTypeContext().getUnknown();
        return;
    }
        ...
        memberAccess && memberAccess->intrinsicMember == IntrinsicMember::PackToStaticArray)
    {
        ...
        {
            WIO_LOG_ADD_ERROR(node.location(),
                              "Pack ToStaticArray requires exactly one explicit element type argument.");
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        }
        ...
        finalizeCallResultType(Compiler::get().getTypeContext().getOrCreateArrayType(
            explicitTypeArguments.front(), ArrayType::ArrayKind::Static, arraySize));
        return;
    }
    ...
    if (calleeSym && calleeSym->kind == SymbolKind::TypeAlias)
    {
        ...
        {
            const size_t fixedArgumentCount =
                getMinimumGenericArgumentCount(calleeSym->genericParameterNames, calleeSym->hasGenericParameterPack);
            const size_t requiredArgumentCount =
                getRequiredGenericArgumentCount(calleeSym->genericParameterDefaults, fixedArgumentCount);
            auto completedArguments =
                completeGenericTypeArguments(calleeSym->genericParameterNames, calleeSym->genericParameterDefaults,
                                             calleeSym->hasGenericParameterPack, explicitTypeArguments);
            if (!completedArguments)
            {
                WIO_LOG_ADD_ERROR(
                    node.location(), "Type alias '{}' expects {} generic arguments, but got {}.", calleeSym->name,
                    calleeSym->hasGenericParameterPack ? common::formatString("at least {}", requiredArgumentCount)
            ...
            constructorGenericBindingSet = buildExtendedGenericBindings(
                calleeSym->genericParameterNames, calleeSym->hasGenericParameterPack, explicitTypeArguments);
            ...
            resolvedAliasResultType = Compiler::get().getTypeContext().getOrCreateAliasType(
                formatAppliedTypeName(calleeSym->name, explicitTypeArguments), resolvedAliasTargetType);
        }
        else
        {
            if (!explicitTypeArguments.empty())
            {
                WIO_LOG_ADD_ERROR(node.location(), "Type alias '{}' does not accept generic arguments.",
                                  calleeSym->name);
                node.refType = Compiler::get().getTypeContext().getUnknown();
                return;
            }
            ...
            resolvedAliasTargetType =
                calleeSym->aliasTargetType ? calleeSym->aliasTargetType : unwrapAliasType(calleeSym->type);
        }
        ...
        if (resolvedConstructorTarget && resolvedConstructorTarget->kind() == TypeKind::Struct)
        {
            ...
            {
                constructorGenericBindings = buildGenericTypeBindings(constructorStructType->genericParameterNames,
                                                                      constructorStructType->genericArguments);
                constructorGenericBindingSet = buildExtendedGenericBindings(
                    constructorStructType->genericParameterNames, constructorStructType->hasGenericParameterPack,
                    constructorStructType->genericArguments);
            }
    ...
    if (calleeSym && calleeSym->kind == SymbolKind::Struct)
    {
        ...
        {
            const size_t fixedArgumentCount =
                getMinimumGenericArgumentCount(structType->genericParameterNames, structType->hasGenericParameterPack);
            if (!explicitTypeArguments.empty())
            {
                const size_t requiredArgumentCount =
                    getRequiredGenericArgumentCount(structType->genericParameterDefaults, fixedArgumentCount);
                auto completedArguments = completeGenericTypeArguments(
                    structType->genericParameterNames, structType->genericParameterDefaults,
                    structType->hasGenericParameterPack, explicitTypeArguments);
                if (!completedArguments)
                {
                    WIO_LOG_ADD_ERROR(node.location(), "Generic type '{}' expects {} generic arguments, but got {}.",
                                      structType->name,
            ...
            if (!explicitTypeArguments.empty())
            {
                constructorGenericBindings =
                    buildGenericTypeBindings(structType->genericParameterNames, explicitTypeArguments);
                constructorGenericBindingSet = buildExtendedGenericBindings(
                    structType->genericParameterNames, structType->hasGenericParameterPack, explicitTypeArguments);
                auto attributeIt = attributeListsBySymbol_.find(genericOwnerSym.Get());
                if (attributeIt != attributeListsBySymbol_.end() && attributeIt->second &&
                    !matchesApplyConstraints(*attributeIt->second, constructorGenericParameterNames,
                                             structType->hasGenericParameterPack, explicitTypeArguments))
                {
                    WIO_LOG_ADD_ERROR(node.location(),
                                      "Generic type '{}' rejects type arguments {} because of @Apply constraints.",
                                      structType->name, formatConcreteInstantiationSignature(explicitTypeArguments));
                    node.refType = Compiler::get().getTypeContext().getUnknown();
                    return;
                }
            ...
            else if (currentExpectedExpressionType_)
            {
                ...
                if (expectedType && expectedType->kind() == TypeKind::Struct)
                {
                    ...
                    if (expectedStructType && expectedStructType->name == structType->name &&
                             ...
                             : expectedStructType->genericArguments.size() == structType->genericParameterNames.size()))
                    {
                        ...
                        {
                            if (!genericArgument || genericArgument->isUnknown() ||
                                containsGenericParameterType(genericArgument))
                            {
                        ...
                        {
                            constructorGenericBindings = buildGenericTypeBindings(structType->genericParameterNames,
                                                                                  expectedStructType->genericArguments);
                            constructorGenericBindingSet = buildExtendedGenericBindings(
                                structType->genericParameterNames, structType->hasGenericParameterPack,
                                expectedStructType->genericArguments);
                            auto attributeIt = attributeListsBySymbol_.find(genericOwnerSym.Get());
                            if (attributeIt != attributeListsBySymbol_.end() && attributeIt->second &&
                                !matchesApplyConstraints(*attributeIt->second, constructorGenericParameterNames,
                            ...
                            structReturnType = instantiateGenericStructType(
                                structType, expectedStructType->genericArguments, node.location());
                            node.callee->refType = structReturnType;
                        }
                    }
                }
    ...
    for (size_t i = 0; i < node.arguments.size(); ++i)
    {
        ...
        if (packExpansionIndex.has_value())
        {
            WIO_LOG_ADD_ERROR(node.arguments[i]->location(),
                              "Only one pack expansion argument is supported in a function call.");
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        }
    ...
    auto isSafeRefCast = [&](const Ref<Type>& dest, const Ref<Type>& src) -> bool
    {
        if (dest && src && dest->kind() == TypeKind::Reference && src->kind() == TypeKind::Reference)
        {
            ...
            if (!dRef->isMutable && isCompatible)
                return true;
            if (dRef->isMutable && sRef->isMutable && isCompatible)
                return true;
        }
        return false;
    };
    ...
    auto isImplicitObjectViewBridge = [&](const Ref<Type>& dest, const Ref<Type>& src) -> bool
    {
        ...
        return expectedTarget->isCompatibleWith(resolvedSrc) || isTypeDerivedFrom(resolvedSrc, expectedTarget);
    };
    ...
    auto analyzeArgumentWithExpectedType = [&](const NodePtr<Expression>& argument, const Ref<Type>& expectedType,
                                                      ...
                                                      const Ref<FunctionType>& callableType) -> size_t
    {
        ...
        const bool hidesExtensionReceiver = declaration->isExtensionMethod && callableType &&
                                                ...
                                                bool usesPackExpansion) -> Ref<FunctionType>
    {
        ...
        if (!functionType->hasParameterPack)
        {
            ...
            return Compiler::get()
                .getTypeContext()
                .getOrCreateFunctionType(functionType->returnType, surfaceParamTypes)
                .AsFast<FunctionType>();
        }
        ...
        if (usesPackExpansion)
        {
            ...
            return Compiler::get()
                .getTypeContext()
                .getOrCreateFunctionType(functionType->returnType, surfaceParamTypes, true)
                .AsFast<FunctionType>();
        }
        ...
        return Compiler::get()
            .getTypeContext()
            .getOrCreateFunctionType(functionType->returnType, surfaceParamTypes, false)
            .AsFast<FunctionType>();
    };

    auto analyzeArgumentsForResolvedFunctionType =
        [&](const Ref<FunctionType>& functionType, const FunctionDeclaration* functionDeclaration,
            bool suppressDiagnostics, bool requireExactArity) -> std::optional<std::vector<Ref<Type>>>
    {
        if (!functionType)
            return std::nullopt;
        ...
        const size_t requiredArgumentCount =
            functionDeclaration ? getRequiredArgumentCountForDeclaration(functionDeclaration, functionType)
    ...
    auto getUncontextualizedArgumentTypes = [&]() -> const std::vector<Ref<Type>>&
    {
        if (!cachedUncontextualizedArgumentTypes.has_value())
        {
            ...
            for (const auto& argument : node.arguments)
            {
                ...
                argumentTypes.push_back(analyzedType.has_value() ? *analyzedType
                                                                 : Compiler::get().getTypeContext().getUnknown());
            }
    ...
    auto scoreResolvedCall = [&](const Ref<FunctionType>& functionType,
                                 const std::vector<Ref<Type>>& actualArgumentTypes, size_t requiredArgumentCount,
                                 bool isGenericCandidate, size_t genericParameterCount) -> std::optional<int>
    {
        if (!functionType || actualArgumentTypes.size() < requiredArgumentCount ||
        ...
        for (size_t i = 0; i < actualArgumentTypes.size(); ++i)
        {
            ...
            {
                currentScore +=
                    actualArgumentTypes[i] && actualArgumentTypes[i]->kind() == TypeKind::GenericParameterPack ? 4 : 8;
                continue;
            }
            ...
            if (dest->isCompatibleWith(src) || (dest->isNumeric() && src->isNumeric()))
            {
                ...
                else if (dest->kind() == TypeKind::Primitive && src->kind() == TypeKind::Primitive)
                {
                    ...
                    auto getSize = [](const std::string& s) -> int
                    {
                        ...
                        if (s.ends_with("32"))
                            return 4;
                        if (s.ends_with("64") || s == "isize" || s == "usize")
                            return 8;
                        return 4;
                    };
    ...
    else if (calleeSym && calleeSym->kind == SymbolKind::Function)
    {
        ...
        requiresOverloadResolution =
            (!calleeSym->genericParameterNames.empty() && containsGenericParameterType(resolvedCalleeType)) ||
            useExplicitFunctionTypeArguments || (isConstructorCall && !constructorGenericParameterNames.empty()) ||
            !constructorGenericBindings.empty() || !constructorGenericBindingSet.packBindings.empty();
    }
    ...
    if (!requiresOverloadResolution)
    {
        for (const auto& candidateSymbol : candidateSymbols)
        {
            ...
            if (candidateDeclaration && getRequiredArgumentCountForDeclaration(
                                            candidateDeclaration, candidateSymbol->type.AsFast<FunctionType>()) !=
    ...
    if (requiresOverloadResolution)
    {
        ...
        auto formatCandidateSignature = [&](const Ref<Symbol>& overload) -> std::string
        {
            ...
            const std::string callableName = isConstructorCall && constructorStructType ? constructorStructType->name
                                             : isCallOperatorInvocation                 ? std::string("operator()")
                                                                                        : overload->name;

            return formatFunctionDiagnosticSignature(callableName, genericParameterNames, functionType,
                                                     isConstructorCall, overload->hasGenericParameterPack);
        };
        ...
        auto isAllowedInstantiateBinding =
            [&](const Ref<Symbol>& overload, const std::vector<std::string>& activeGenericParameterNames,
                const bool hasActiveGenericParameterPack, const GenericBindingSet& bindings) -> bool
        {
            ...
            auto resolvedBindingTypes = tryMaterializeConcreteInstantiation(activeGenericParameterNames,
                                                                            hasActiveGenericParameterPack, bindings);
        ...
        auto satisfiesApplyBinding =
            [&](const Ref<Symbol>& symbol, const std::vector<std::string>& activeGenericParameterNames,
                const bool hasActiveGenericParameterPack, const GenericBindingSet& bindings) -> bool
        {
            ...
            return matchesApplyConstraints(*attributeIt->second, activeGenericParameterNames,
                                           hasActiveGenericParameterPack, bindings);
        };

        auto satisfiesOpenNativeTemplateIntrinsicBinding =
            [&](const Ref<Symbol>& symbol, const std::vector<std::string>& activeGenericParameterNames,
                const bool hasActiveGenericParameterPack, const GenericBindingSet& bindings) -> bool
        {
            if (!symbol || activeGenericParameterNames.empty())
        ...
        auto tryResolveFunctionCandidate = [&](const Ref<Symbol>& overload) -> std::optional<ResolvedFunctionCandidate>
        {
            ...
            const std::vector<Ref<Type>>& genericParameterDefaults =
                isConstructorCall && constructorStructType ? constructorStructType->genericParameterDefaults
                                                           : overload->genericParameterDefaults;
            GenericBindingSet bindingSet;
            bool usedGenericDefaults = false;
            if (isConstructorCall && (!constructorGenericBindingSet.directBindings.empty() ||
            ...
            if (useExplicitFunctionTypeArguments)
            {
                ...
                auto completedExplicitArguments =
                    completeGenericTypeArguments(activeGenericParameterNames, genericParameterDefaults,
                                                 hasGenericParameterPack, explicitTypeArguments);
                ...
                {
                    if (!explicitTypeArgument || explicitTypeArgument->isUnknown())
                    {
                        return std::nullopt;
                    }
                }

                bindingSet = buildExtendedGenericBindings(activeGenericParameterNames, hasGenericParameterPack,
            ...
            if (useExplicitFunctionTypeArguments && hasGenericParameterPack && declaredFunctionHadParameterPack &&
                !candidateHasParameterPack && !activeGenericParameterNames.empty())
            {
            ...
            else if (node.arguments.size() < requiredArgumentCount ||
                     (!candidateHasParameterPack && node.arguments.size() > declaredFunctionType->paramTypes.size()))
            {
                return std::nullopt;
            }
            ...
            if (isGenericCandidate)
                collectGenericParameterInstances(resolvedFunctionType, activeGenericParameterNames,
                                                 activeGenericParameterInstances);
            ...
            auto getExpectedParameterType = [&](size_t index) -> Ref<Type>
            {
                ...
                if (!bindingSet.directBindings.empty() || !bindingSet.packBindings.empty() ||
            ...
            for (size_t i = 0; i < node.arguments.size(); ++i)
            {
                ...
                const bool isConcretePackTailArgument =
                    candidateHasParameterPack && !packExpansionIndex.has_value() && i >= fixedParameterCount;
                bool shouldDeferArgument =
                    isGenericCandidate && !usesPackExpansionArgument && !isConcretePackTailArgument &&
                ...
                if (isGenericCandidate && !usesPackExpansionArgument && !isConcretePackTailArgument &&
            ...
            const size_t fixedGenericParameterCount =
            ...
            for (size_t genericIndex = 0; genericIndex < fixedGenericParameterCount; ++genericIndex)
            {
                ...
                if (bindings.contains(parameterName) && bindings.at(parameterName) &&
                    !bindings.at(parameterName)->isUnknown())
                    continue;
            ...
            if (isGenericCandidate)
            {
                ...
                {
                    if (!bindings.contains(genericParameterName) || !bindings.at(genericParameterName) ||
            ...
            bindingSet.directBindings = bindings;
            if (hasGenericParameterPack && candidateHasParameterPack && !activeGenericParameterNames.empty() &&
                    ...
                    if (candidateArgTypes.size() > fixedParameterCount)
                    {
                        ...
                        for (size_t i = fixedParameterCount; i < candidateArgTypes.size(); ++i)
                        {
                            ...
                            bindingSet.directBindings.emplace(
                                makePackElementBindingName(packName, i - fixedParameterCount), candidateArgTypes[i]);
                        }
                    }
            ...
            {
                if (!isAllowedInstantiateBinding(overload, activeGenericParameterNames, hasGenericParameterPack,
                                                 bindingSet))
                {
                    auto resolvedBindingTypes = tryMaterializeConcreteInstantiation(
                        activeGenericParameterNames, hasGenericParameterPack, bindingSet);
                ...
                if (!satisfiesApplyBinding(applyConstraintOwner, activeGenericParameterNames, hasGenericParameterPack,
                                           bindingSet))
                {
                    auto resolvedBindingTypes = tryMaterializeConcreteInstantiation(
                        activeGenericParameterNames, hasGenericParameterPack, bindingSet);
                ...
                if (!satisfiesOpenNativeTemplateIntrinsicBinding(overload, activeGenericParameterNames,
                                                                 hasGenericParameterPack, bindingSet))
                {
                    auto resolvedBindingTypes = tryMaterializeConcreteInstantiation(
                        activeGenericParameterNames, hasGenericParameterPack, bindingSet);
            ...
            if (auto score = scoreResolvedCall(resolvedFunctionType.AsFast<FunctionType>(), candidateArgTypes,
                                               requiredArgumentCount, remainsGenericCandidate,
                                               activeGenericParameterNames.size());
                score.has_value())
            {
                auto callableSurfaceType = buildCallableSurfaceFunctionType(
                    resolvedFunctionType.AsFast<FunctionType>(), candidateArgTypes, packExpansionIndex.has_value());
                return ResolvedFunctionCandidate{.symbol = overload,
                                                 .functionType =
                                                     callableSurfaceType ? callableSurfaceType : resolvedFunctionType,
        ...
        if (isAmbiguous)
        {
            ...
            {
                WIO_LOG_ADD_ERROR(node.location(), "Ambiguous function call to '{}' with arguments {}.",
                                  getCallableDisplayName(), formatCallArgumentTypesForDiagnostic());
            }
            else
            {
                WIO_LOG_ADD_ERROR(node.location(), "Ambiguous function call to '{}' with arguments {}. Candidates: {}.",
                                  getCallableDisplayName(), formatCallArgumentTypesForDiagnostic(), candidateSummary);
            }
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        }
        if (!bestMatch.has_value())
        {
            ...
            {
                WIO_LOG_ADD_ERROR(node.location(), "Generic interop function '{}' does not declare @Instantiate{}.",
                                  rejectedInstantiationFunctionName, rejectedInstantiationSignature);
            }
            else if (rejectedByApplyConstraints)
            {
                WIO_LOG_ADD_ERROR(node.location(),
                                  "Generic callable '{}' rejects type arguments {} because of @Apply constraints.",
                                  rejectedApplyTargetName, rejectedApplySignature);
            }
            else if (rejectedByOpenNativeTemplateIntrinsic)
            {
                WIO_LOG_ADD_ERROR(node.location(),
                                  "Generic native callable '{}' accepts only enum or flagset type arguments. Got {}.",
                                  rejectedOpenNativeTemplateTargetName, rejectedOpenNativeTemplateSignature);
            }
            else
            {
                const auto availableGenericArities = buildAvailableGenericAritySummary();
                const bool providedExplicitTypeArguments =
                    useExplicitFunctionTypeArguments || (isConstructorCall && !explicitTypeArguments.empty());
                ...
                if (providedExplicitTypeArguments && !availableGenericArities.empty() &&
                    !anyCandidateAcceptsExplicitTypeArity)
                {
                    ...
                    for (const auto& candidate : candidateSymbols)
                    {
                        ...
                        if (candidate->hasGenericParameterPack)
                            appendUniqueValue(minimumPackArities,
                                              getMinimumGenericArgumentCount(genericParameterNames, true));
                        else
                    ...
                    if (exactArities.empty() && minimumPackArities.size() == 1)
                    {
                        WIO_LOG_ADD_ERROR(
                            ...
                            getCallableDisplayName(), minimumPackArities.front(), explicitTypeArguments.size());
                    }
                    else if (availableGenericArities.size() == 1 && minimumPackArities.empty())
                    {
                        WIO_LOG_ADD_ERROR(
                            node.location(), "Generic call to '{}' expects {} explicit type arguments, but got {}.",
                            getCallableDisplayName(), availableGenericArities.front(), explicitTypeArguments.size());
                    }
                    else
                    {
                        ...
                        WIO_LOG_ADD_ERROR(
                            ...
                            getCallableDisplayName(), explicitTypeArguments.size(), arityList);
                    }
                }
                else
                {
                    ...
                    if (candidateSummary.empty())
                    {
                        WIO_LOG_ADD_ERROR(node.location(), "No matching overload for '{}' with arguments {}.",
                                          getCallableDisplayName(), formatCallArgumentTypesForDiagnostic());
                    }
                    else
                    {
                        WIO_LOG_ADD_ERROR(
                            node.location(), "No matching overload for '{}' with arguments {}. Candidates: {}.",
                            getCallableDisplayName(), formatCallArgumentTypesForDiagnostic(), candidateSummary);
                    }
                }
            }
        ...
        if (isConstructorCall && constructorStructType && !constructorGenericParameterNames.empty() &&
            constructorGenericBindings.empty() && constructorGenericBindingSet.packBindings.empty())
        {
            auto deducedGenericArguments = tryMaterializeConcreteInstantiation(
                constructorGenericParameterNames, constructorStructType->hasGenericParameterPack,
            ...
            structReturnType =
                instantiateGenericStructType(constructorStructType, *deducedGenericArguments, node.location());
            node.callee->refType = structReturnType;
        }
        ...
        auto getConcreteCallableOwnerType = [&]() -> Ref<StructType>
        {
            auto resolveStructReceiverType = [&](const Ref<Type>& candidateType) -> Ref<StructType>
            {
                ...
                return resolvedType && resolvedType->kind() == TypeKind::Struct ? resolvedType.AsFast<StructType>()
                                                                                : nullptr;
            };
        ...
        if (bestMatch->symbol)
        {
            if (!isConstructorCall && bestMatch->usedGenericDefaults &&
                !bestMatch->symbol->genericParameterNames.empty())
            {
                if (auto resolvedArguments = tryMaterializeConcreteInstantiation(
                        bestMatch->symbol->genericParameterNames, bestMatch->symbol->hasGenericParameterPack,
                        bestMatch->bindingSet))
                {
            ...
            Ref<Symbol> validationSymbol =
                bestMatch->symbol->flags.get_isExtension() && bestMatch->symbol->extensionImplementation
                    ? bestMatch->symbol->extensionImplementation
                    : bestMatch->symbol;
            const FunctionDeclaration* validationDeclaration = getFunctionDeclarationForSymbol(validationSymbol);
            ...
            {
                Ref<Type> instantiatedValidationType =
                    instantiateGenericType(validationSymbol->type, bestMatch->bindingSet);
            ...
            if (validationDeclaration && validationFunctionType &&
                !validateConcreteGenericFunctionBody(*validationDeclaration, validationSymbol, validationFunctionType,
                                                     concreteOwnerType, bestMatch->bindingSet.directBindings,
        ...
        if (auto resolvedArgumentTypes = analyzeArgumentsForResolvedFunctionType(
                bestMatch->functionType.AsFast<FunctionType>(), nullptr, false, false);
        ...
        if (!finalizeCallResultType(isConstructorCall ? structReturnType
                                                      : bestMatch->functionType.AsFast<FunctionType>()->returnType))
            return;
        return;
    }
        ...
        memberAccess && !memberAccess->intrinsicOverloadMembers.empty())
    {
        ...
        for (size_t i = 0; i < memberAccess->intrinsicOverloadTypes.size(); ++i)
        {
            ...
            auto overloadArgumentTypes =
                analyzeArgumentsForResolvedFunctionType(overloadType.AsFast<FunctionType>(), nullptr, true, true);
            if (!overloadArgumentTypes.has_value())
                continue;

            auto score = scoreResolvedCall(overloadType.AsFast<FunctionType>(), *overloadArgumentTypes,
                                           overloadType.AsFast<FunctionType>()->paramTypes.size(), false, 0);
            if (!score.has_value())
                continue;
        ...
        if (isAmbiguous)
        {
            ...
            WIO_LOG_ADD_ERROR(node.location(), "Ambiguous function call to intrinsic member '{}' with arguments {}.",
                              memberAccess->member->token.value, formatCallArgumentTypesForDiagnostic());
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        }
        ...
        if (!bestIndex.has_value())
        {
            ...
            WIO_LOG_ADD_ERROR(node.location(), "No matching overload for intrinsic member '{}' with arguments {}.",
                              memberAccess->member->token.value, formatCallArgumentTypesForDiagnostic());
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        }
        ...
        if (auto resolvedArgumentTypes =
                analyzeArgumentsForResolvedFunctionType(calleeType.AsFast<FunctionType>(), nullptr, false, false);
            !resolvedArgumentTypes.has_value())
        {
            node.refType = Compiler::get().getTypeContext().getUnknown();
            return;
        ...
        if (isMutatingIntrinsicMember(memberAccess->intrinsicMember) &&
        ...
        {
            WIO_LOG_ADD_ERROR(node.location(), node.isPipelineCall ? "Pipeline target is not callable."
    ...
    if (!calleeSym && (!calleeType || calleeType->kind() != TypeKind::Function))
    {
        ...
        WIO_LOG_ADD_ERROR(node.location(),
                          node.isPipelineCall ? "Pipeline target is not callable." : "Called expression is undefined.");
        node.refType = Compiler::get().getTypeContext().getUnknown();
        return;
    }
    ...
    if (auto resolvedArgumentTypes =
            analyzeArgumentsForResolvedFunctionType(funcType, directFunctionDeclaration, false, false);
        resolvedArgumentTypes.has_value())
    {
    ...
    else
    {
        ...
        if (node.arguments.size() < requiredArgumentCount ||
            (!funcType->hasParameterPack && node.arguments.size() > totalParameterCount))
        {
    ...
    const size_t requiredArgumentCount = getRequiredArgumentCountForCallable(calleeSym, funcType);
    if (argTypes.size() < requiredArgumentCount ||
        (!funcType->hasParameterPack && argTypes.size() > funcType->paramTypes.size()))
        WIO_LOG_ADD_ERROR(node.location(), "Function expects '{}' arguments, but got '{}'.",
                          funcType->hasParameterPack ? common::formatString("{} or more", requiredArgumentCount)
                                                     : formatExpectedArgumentCountDescription(
                                                           requiredArgumentCount, funcType->paramTypes.size()),
                          argTypes.size());
    ...
    size_t argCount = funcType->hasParameterPack ? std::min(argTypes.size(), fixedParameterCount)
                                                 : std::min(argTypes.size(), funcType->paramTypes.size());
    for (size_t i = 0; i < argCount; ++i)
    {
        ...
        if (!isAssignmentLikeCompatible(expectedType, actualType) &&
            !isImplicitObjectViewBridge(expectedType, actualType) && !isSafeRefCast(expectedType, actualType))
        {
            ...
            if (expectedType->kind() == TypeKind::Reference && actualType->kind() == TypeKind::Reference)
            {
                ...
                {
                    extraHint = " Hint: The function expects a mutable reference ('ref'), but a read-only reference "
                                "('view') or immutable variable ('let') was provided.";
                }
                else if (!isTypeDerivedFrom(actRef->referredType, expRef->referredType))
                {
                    extraHint = " Hint: Type '" + actRef->referredType->toString() + "' does not inherit from '" +
                                expRef->referredType->toString() + "'.";
                }
            }

            WIO_LOG_ADD_ERROR(node.arguments[i]->location(),
                              "Argument mismatch at index {}: Expected '{}', but got '{}'.{}", i,
                              expectedType->toString(), actualType->toString(), extraHint);
        }
    }
