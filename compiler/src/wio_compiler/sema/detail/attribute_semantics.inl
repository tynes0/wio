...
void SemanticAnalyzer::visit(DeclarationGroup& node)
{
    for (auto& declaration : node.declarations)
    {
        if (!declaration)
            continue;
        if (isAttributeContractPass_ && !declaration->is<AttributeDeclaration>() &&
            !declaration->is<DeclarationGroup>() && !declaration->is<RealmDeclaration>())
        {
            continue;
        }
        if (isDeriveExpansionPass_ && !declaration->is<ComponentDeclaration>() &&
            !declaration->is<ObjectDeclaration>() && !declaration->is<DeclarationGroup>() &&
...
void SemanticAnalyzer::validateAttributeApplications(std::vector<NodePtr<AttributeStatement>>& attributes,
                                                     std::string_view target, bool validateTarget)
{
    ...
    for (const auto& application : attributes)
    {
        ...
        if (application->attribute != Attribute::Unknown)
        {
                ...
                std::ranges::find(contract->targets, std::string_view(target)) == contract->targets.end())
            {
                ...
                WIO_LOG_ADD_ERROR(application->location(), "Attribute '{}' cannot target '{}'. Allowed targets: {}.",
                                  contract->canonicalName, target, allowedTargets);
            }

            const size_t count = ++builtinApplicationCounts[application->attribute];
            if (count > 1 && !contract->repeatable)
            {
                WIO_LOG_ADD_ERROR(application->location(), "Attribute '{}' is not repeatable on the same declaration.",
    ...
    for (size_t attributeIndex = 0; attributeIndex < attributes.size(); ++attributeIndex)
    {
        ...
        if (!symbol || symbol->kind != SymbolKind::Attribute)
        {
            WIO_LOG_ADD_ERROR(attribute->location(),
        ...
        for (size_t processorIndex = 0; processorIndex < symbol->attributeProcessorPhases.size(); ++processorIndex)
        {
            attribute->processorBindings.push_back(AttributeStatement::ProcessorBinding{
                ...
                .hookMode = processorIndex < symbol->attributeProcessorHookModes.size()
                                ...
                                : std::string{},
                .processorType = processorIndex < symbol->attributeProcessorResolvedTypes.size()
                                     ? symbol->attributeProcessorResolvedTypes[processorIndex]
                                     : nullptr,
                .hookSymbol = processorIndex < symbol->attributeProcessorHookSymbols.size()
                                  ? symbol->attributeProcessorHookSymbols[processorIndex]
                                  : nullptr,
        ...
        for (size_t processorIndex = 0; processorIndex < symbol->attributeProcessorPhases.size(); ++processorIndex)
        {
            ...
            const bool targetIsAny = validatorTarget && validatorTarget->kind() == TypeKind::Primitive &&
            ...
            if (validatorTarget && !targetIsAny && target != "component" && target != "object")
            {
                WIO_LOG_ADD_ERROR(
                    ...
                    validatorTarget->toString(), target);
            }
        }

        attribute->runtimeRetained =
            std::ranges::find(symbol->attributeRetention, std::string("runtime")) != symbol->attributeRetention.end();
        ...
        for (size_t processorIndex = 0; processorIndex < symbol->attributeProcessorPhases.size(); ++processorIndex)
        {
            ...
            const std::string diagnostic =
                processorIndex < symbol->attributeProcessorDiagnostics.size() &&
                        !symbol->attributeProcessorDiagnostics[processorIndex].empty()
                    ? symbol->attributeProcessorDiagnostics[processorIndex]
                    : common::formatString("Attribute validator '{}' rejected application of '{}'.", processorName,
        ...
        for (Token& argument : attribute->args)
        {
            ...
            if (declarationIt == variableDeclarationsBySymbol_.end() || !declarationIt->second ||
            ...
            if (auto folded = tryEvaluateStaticAttributeConstant(declarationIt->second->initializer,
                                                                 variableDeclarationsBySymbol_, activeSymbols))
            {
        ...
        const bool hasNamedArguments =
            std::ranges::any_of(attribute->argumentNames, [](const std::string& name) { return !name.empty(); });
        if (hasNamedArguments)
        {
            if (attribute->argumentNames.size() != attribute->args.size() ||
                attribute->typeArgs.size() != attribute->args.size())
            {
                WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' has inconsistent argument metadata.",
            ...
            for (size_t sourceIndex = 0; sourceIndex < attribute->args.size(); ++sourceIndex)
            {
                ...
                {
                    while (nextPositionalIndex < assignedSources.size() &&
                           assignedSources[nextPositionalIndex].has_value())
                        ++nextPositionalIndex;
                    parameterIndex = nextPositionalIndex++;
                    if (parameterIndex >= assignedSources.size())
                    {
                        WIO_LOG_ADD_ERROR(attribute->args[sourceIndex].loc,
                ...
                else
                {
                    ...
                    {
                        WIO_LOG_ADD_ERROR(attribute->args[sourceIndex].loc,
                                          "Attribute '{}' has no parameter named '{}'.", attribute->qualifiedName,
                                          argumentName);
                        invalidNamedArguments = true;
                        continue;
                    }
                    parameterIndex =
                        static_cast<size_t>(std::distance(symbol->attributeParameterNames.begin(), parameter));
                }
            ...
            while (requiredArgumentCount > 0 && symbol->attributeParameterHasDefault[requiredArgumentCount - 1])
            {
            ...
            for (size_t parameterIndex = 0; parameterIndex < requiredArgumentCount; ++parameterIndex)
            {
                if (!assignedSources[parameterIndex].has_value())
                {
                    WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' is missing required argument '{}'.",
                                      attribute->qualifiedName, symbol->attributeParameterNames[parameterIndex]);
                    invalidNamedArguments = true;
                }
            }
        ...
        const bool targetAllowed =
            !validateTarget ||
            std::ranges::find(symbol->attributeTargets, std::string(target)) != symbol->attributeTargets.end();
        if (validateTarget && !targetAllowed)
        {
            ...
            WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' cannot target '{}'. Allowed targets: {}.",
                              attribute->qualifiedName, target, allowedTargets);
        }

        const size_t applicationCount = ++applicationCounts[symbol.Get()];
        if (applicationCount > 1 && !symbol->attributeRepeatable)
        {
            WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' is not repeatable on the same declaration.",
                              attribute->qualifiedName);
        }
        ...
        for (const auto& group : symbol->attributeConflictGroups)
        {
            ...
            if (!inserted && owner->second != symbol.Get())
            {
                WIO_LOG_ADD_ERROR(attribute->location(),
                                  ...
                                  attribute->qualifiedName, owner->second->name, group);
            }
        }
        ...
        {
            WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' expects {} to {} arguments, but got {}.",
                              attribute->qualifiedName, requiredArgumentCount, symbol->attributeParameterTypes.size(),
                              attribute->args.size());
            continue;
        }
        ...
        while (attribute->args.size() < symbol->attributeParameterTypes.size())
        {
            ...
            if (parameterIndex >= symbol->attributeParameterDefaults.size() ||
                !symbol->attributeParameterDefaults[parameterIndex].isValid())
            {
                WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' could not materialize default argument '{}'.",
                                  attribute->qualifiedName,
            ...
            attribute->argumentNames.push_back(parameterIndex < symbol->attributeParameterNames.size()
                                                   ? symbol->attributeParameterNames[parameterIndex]
                                                   : std::string{});
        ...
        for (size_t parameterIndex = 0; parameterIndex < attribute->argumentNames.size() &&
                                        parameterIndex < symbol->attributeParameterNames.size();
             ++parameterIndex)
        {
        ...
        for (size_t index = 0; index < attribute->args.size(); ++index)
        {
            ...
            if (!compatible)
            {
                WIO_LOG_ADD_ERROR(argument.loc, "Argument {} of attribute '{}' must be '{}'.", index + 1,
                                  attribute->qualifiedName, expectedType->toString());
            }
        }
            ...
            for (const auto& composedTemplate : symbol->attributeComposition)
            {
                    ...
                    std::ranges::find(chain, composedSymbol.Get()) != chain.end())
                {
                    WIO_LOG_ADD_ERROR(attribute->location(),
                                      ...
                                      attribute->qualifiedName, composedTemplate->qualifiedName);
                    continue;
                }
                ...
                for (size_t argumentIndex = 0; argumentIndex < composedArguments.size(); ++argumentIndex)
                {
                    ...
                    const auto parameter =
                        std::ranges::find(symbol->attributeParameterNames, composedArguments[argumentIndex].value);
                    if (parameter == symbol->attributeParameterNames.end())
                        continue;
                    ...
                    if (argumentIndex < composedTypeArguments.size() && parameterIndex < attribute->typeArgs.size())
                    {
                ...
                auto expanded = makeNodePtr<AttributeStatement>(
                    composedTemplate->attribute, std::move(composedArguments), std::move(composedTypeArguments),
                    attribute->location(), composedTemplate->qualifiedName, composedTemplate->argumentNames);
    ...
    auto applicationMatches = [&](const NodePtr<AttributeStatement>& application, std::string_view requiredName)
    {
        if (!application)
            return false;
    ...
    for (const auto& attribute : attributes)
    {
        ...
        for (const std::string_view required : contract->requiredAttributes)
        {
            if (!hasEffectiveAttribute(required))
                WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' requires attribute '{}'.",
                                  contract->canonicalName, required);
        }
        ...
        for (const std::string_view conflict : contract->conflictingAttributes)
        {
            if (hasEffectiveAttribute(conflict))
                WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' conflicts with attribute '{}'.",
                                  contract->canonicalName, conflict);
        }
    }
    ...
    for (const auto& attribute : attributes)
    {
            ...
            (count < symbol->attributeCardinalityMin || count > symbol->attributeCardinalityMax))
        {
            WIO_LOG_ADD_ERROR(
                ...
                attribute->qualifiedName, symbol->attributeCardinalityMin, symbol->attributeCardinalityMax, count);
        }
        ...
        for (const std::string& required : symbol->attributeRequiredAttributes)
        {
            if (!hasEffectiveAttribute(required))
                WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' requires attribute '{}'.",
                                  attribute->qualifiedName, required);
        }
        ...
        for (const std::string& conflict : symbol->attributeConflictingAttributes)
        {
            if (hasEffectiveAttribute(conflict))
                WIO_LOG_ADD_ERROR(attribute->location(), "Attribute '{}' conflicts with attribute '{}'.",
                                  attribute->qualifiedName, conflict);
        }
        if (!symbol->attributeOnlyWithAttributes.empty())
        {
            for (const auto& other : attributes)
            {
                if (!other || other.Get() == attribute.Get() || applicationMatches(other, attribute->qualifiedName))
                    continue;
                const bool allowed =
                    std::ranges::any_of(symbol->attributeOnlyWithAttributes, [&](const std::string& allowedName)
                                        { return applicationMatches(other, allowedName); });
                if (!allowed)
                {
                    WIO_LOG_ADD_ERROR(other->location(),
                                      "Attribute '{}' is not permitted alongside '{}' by its OnlyWith policy.",
                                      other->qualifiedName, attribute->qualifiedName);
                }
            }
        }
    }
    ...
    enum class OrderingVisit : std::uint8_t
    {
        None,
        Active,
        Complete
    ...
    std::function<bool(const std::string&)> visitOrdering = [&](const std::string& name)
    {
        ...
        if (state == OrderingVisit::Active)
        {
            ...
            if (!cycle.empty())
                cycle += " -> ";
            cycle += name;
            WIO_LOG_ADD_ERROR(attributes.empty() ? common::Location::invalid() : attributes.front()->location(),
                              "Attribute processor ordering cycle detected: {}.", cycle);
            return true;
        }
    ...
    for (const auto& attribute : attributes)
    {
        ...
        const std::string name =
            nameTail(attribute->canonicalName.empty() ? attribute->qualifiedName : attribute->canonicalName);
    ...
    for (size_t sourceIndex = 0; sourceIndex < attributes.size(); ++sourceIndex)
    {
        ...
        attribute->processorOrder = rank == processorRanks.end() ? sourceIndex : rank->second;
    }
}

void SemanticAnalyzer::applyActiveScopedAttributes(std::vector<NodePtr<AttributeStatement>>& attributes,
                                                   std::string_view target)
{
    for (const auto& active : activeScopedAttributes_)
    {
        ...
        if (std::ranges::find(symbol->attributeTargets, std::string(target)) == symbol->attributeTargets.end())
            continue;
        const bool alreadyApplied = std::ranges::any_of(attributes,
                                                        [&](const auto& existing)
                                                        {
                                                            if (!existing)
                                                                return false;
                                                            ...
                                                            // scoped application is materialized into the declaration
                                                            // on the first visit, so pointer identity cannot recognize
                                                            // it on later visits. The source location identifies the
                                                            // original `using` application while still allowing a
                                                            // nested, distinct application of the same non-repeatable
                                                            // attribute to be diagnosed normally.
        ...
        {
            auto scoped =
                makeNodePtr<AttributeStatement>(active->attribute, active->args, active->typeArgs, active->location(),
                                                active->qualifiedName, active->argumentNames);
...
void SemanticAnalyzer::registerDerivedMethods(std::vector<NodePtr<AttributeStatement>>& attributes,
                                              const Ref<Type>& targetType, std::string_view target)
{
    ...
    auto findDeriveMemberMarker = [&](const FunctionDeclaration* declaration) -> const AttributeStatement*
    {
        if (!declaration)
            return nullptr;
    ...
    std::function<void(const AttributeStatement*, const Ref<Symbol>&, std::unordered_set<const Symbol*>&)>
        collectEffectiveAttribute = [&](const AttributeStatement* sourceApplication, const Ref<Symbol>& symbol,
    ...
    {
        for (size_t processorIndex = 0; processorIndex < attributeSymbol->attributeProcessorPhases.size();
             ++processorIndex)
        {
            if (attributeSymbol->attributeProcessorPhases[processorIndex] == "validation")
            {
                Ref<Type> validatorTarget =
                    processorIndex < attributeSymbol->attributeProcessorTargetTypes.size()
                        ...
                        : nullptr;
                const bool validatorTargetIsAny = validatorTarget && validatorTarget->kind() == TypeKind::Primitive &&
                                                  validatorTarget.AsFast<PrimitiveType>()->name == "any";
                if (validatorTarget && !validatorTargetIsAny && !isTypeDerivedFrom(resolvedTarget, validatorTarget))
                {
                    WIO_LOG_ADD_ERROR(
                        ...
                        processorIndex < attributeSymbol->attributeProcessorCanonicalTypes.size()
                            ...
                            : std::string("<validator>"),
                        validatorTarget->toString(), attributeSymbol->attributeCanonicalName,
            ...
            const std::string processorName = processorIndex < attributeSymbol->attributeProcessorCanonicalTypes.size()
                                                  ? attributeSymbol->attributeProcessorCanonicalTypes[processorIndex]
                                                  : std::string{};
            ...
            if (Ref<Symbol> constructors = processorSymbol->innerScope->resolveLocally("OnConstruct"))
            {
                ...
                {
                    Ref<FunctionType> constructorType =
                        constructor && constructor->type ? constructor->type.AsFast<FunctionType>() : nullptr;
            ...
            {
                WIO_LOG_ADD_ERROR(application->location(),
                                  "DeriveProcessor '{}' must be default constructible because each derived call owns "
                                  "an isolated processor instance.",
                                  processorName);
                continue;
            }

            const std::string processorCppType = processorIndex < attributeSymbol->attributeProcessorCppTypes.size()
            ...
            if (processorTargetType && !processorTargetIsAny && !isTypeDerivedFrom(resolvedTarget, processorTargetType))
            {
                WIO_LOG_ADD_ERROR(application->location(),
                                  "DeriveProcessor '{}' requires a target compatible with '{}', but attribute '{}' is "
                                  "applied to '{}'.",
                                  processorName, processorTargetType->toString(),
                                  attributeSymbol->attributeCanonicalName, resolvedTarget->toString());
                continue;
            }
            ...
            for (const auto& [_, member] : processorSymbol->innerScope->getSymbols())
            {
                ...
                for (const Ref<Symbol>& methodSymbol : candidates)
                {
                    ...
                    const FunctionDeclaration* declaration =
                        declarationIt == functionDeclarationsBySymbol_.end() ? nullptr : declarationIt->second;
                    const AttributeStatement* marker = findDeriveMemberMarker(declaration);
                    if (!marker)
                        continue;
                    sawDerivedMember = true;

                    Ref<FunctionType> methodType =
                        methodSymbol && methodSymbol->type ? methodSymbol->type.AsFast<FunctionType>() : nullptr;
                    ...
                    const auto receiverReference = receiverType && receiverType->kind() == TypeKind::Reference
                                                       ...
                                                       : nullptr;
                    Ref<Type> typedReceiverTarget =
                        receiverReference ? unwrapAliasType(receiverReference->referredType) : nullptr;
                    const bool receiverMatchesTargetContract =
                        processorTargetType && !processorTargetIsAny && receiverReference &&
                        !receiverReference->isMutable && typedReceiverTarget &&
                        typedReceiverTarget->isCompatibleWith(processorTargetType);
                    const bool hasDefaultedPublicParameter =
                        declaration && std::ranges::any_of(declaration->parameters | std::views::drop(1),
                                                           [](const Parameter& parameter)
                                                           { return parameter.defaultValue != nullptr; });
                    if (!declaration || !methodType || (!receiverIsAny && !receiverMatchesTargetContract) ||
                        !methodSymbol->flags.get_isPublic() || declaration->isAsync ||
                        !declaration->genericParameters.empty() || methodType->hasParameterPack ||
                        hasDefaultedPublicParameter || hasAttribute(declaration->attributes, Attribute::Native))
                    {
                        WIO_LOG_ADD_ERROR(
                            marker->location(),
                            "Derived member '{}.{}' must be a public, synchronous, non-generic Wio method whose first "
                            "parameter is 'any' or an immutable view of its DeriveProcessor target contract, and whose "
                            "public parameters have no defaults or packs.",
                            processorName, methodSymbol ? methodSymbol->name : std::string("<unknown>"));
                        continue;
                    }
                    ...
                    if (exposedName == "OnConstruct" || common::isOperatorOverloadName(exposedName))
                    {
                        WIO_LOG_ADD_ERROR(marker->location(),
                                          "Derived member '{}' cannot define constructors or operators.", exposedName);
                        continue;
                    }

                    if (auto scope = targetStruct->structScope.Lock(); scope && scope->resolveLocally(exposedName))
                    {
                        WIO_LOG_ADD_ERROR(application->location(),
                                          "Derived member '{}' conflicts with an existing member on '{}'.", exposedName,
                                          resolvedTarget->toString());
                        continue;
                    }
                    ...
                    {
                        WIO_LOG_ADD_ERROR(application->location(), "Derived member '{}' is ambiguous for '{}'.",
                                          exposedName, resolvedTarget->toString());
                        continue;
                    }

                    Ref<Symbol> derived =
                        createSymbol(methodSymbol->name, methodType, SymbolKind::Function, application->location());
...
void SemanticAnalyzer::visit(AttributeDeclaration& node)
{
    ...
    if (isDeclarationPass_)
    {
        static const std::unordered_set<std::string> supportedTargets = {
            "fn",        "method",    "component",         "object", "interface",    "type",        "field",
            "variable",  "parameter", "generic_parameter", "enum",   "flagset",      "flag",        "enum_case",
            "extension", "realm",     "attribute",         "module", "import_scope", "constructor", "application",
            "system",    "handler",   "generated"};
        static const std::unordered_set<std::string> supportedRetention = {"source", "compile", "runtime"};
        ...
        for (auto& parameter : node.parameters)
        {
            ...
            if (parameter.defaultValue)
            {
                ...
                if (auto folded = tryEvaluateStaticAttributeConstant(parameter.defaultValue,
                                                                     variableDeclarationsBySymbol_, activeSymbols))
                {
            ...
            if (parameter.defaultValue)
                ...
                WIO_LOG_ADD_ERROR(parameter.name->location(),
                                  "Required attribute parameters cannot follow defaulted parameters.");
        }
        ...
        Ref<Symbol> symbol = createSymbol(node.name->token.value, Compiler::get().getTypeContext().getUnknown(),
                                          SymbolKind::Attribute, node.location());
    ...
    if (!isAttributeContractPass_)
    {
        for (size_t index = 0; index < node.parameters.size(); ++index)
        {
            ...
            if (expectedType && actualType && !expectedType->isUnknown() && !actualType->isUnknown() &&
                !isAssignmentLikeCompatible(expectedType, actualType))
            {
                WIO_LOG_ADD_ERROR(parameter.defaultValue->location(),
            ...
            if (symbol && index < symbol->attributeParameterDefaults.size())
            {
                ...
                {
                    WIO_LOG_ADD_ERROR(parameter.defaultValue->location(),
                                      "Default value for attribute parameter '{}' must be a compile-time scalar, "
                                      "string, or text expression.",
                                      parameter.name ? parameter.name->token.value : std::to_string(index));
                }
            }
        }
    ...
    attributeSymbol->attributeProcessorResolvedTypes.clear();
    ...
    attributeSymbol->attributeProcessorHookSymbols.clear();
    ...
    for (const std::string& processorName : node.processorTypes)
    {
        ...
        if (!processorSymbol || processorSymbol->kind != SymbolKind::Struct || !processorType ||
            processorType->kind() != TypeKind::Struct || !processorType.AsFast<StructType>()->isObject)
        {
            WIO_LOG_ADD_ERROR(node.location(),
        ...
        std::function<void(const Ref<Type>&)> collectPhases = [&](const Ref<Type>& candidate)
        {
            ...
            else if (typeName == "PreProcessor")
                phases.insert("pre");
            else if (typeName == "PostProcessor")
                phases.insert("post");
            else if (typeName == "FinallyProcessor")
                phases.insert("finally");
            else if (typeName == "AroundProcessor")
                phases.insert("around");
        ...
        {
            WIO_LOG_ADD_ERROR(node.location(),
                              "Attribute processor '{}' must implement exactly one of Validator, DeriveProcessor, "
                              "PreProcessor, PostProcessor, FinallyProcessor, or AroundProcessor.",
                              processorName);
            continue;
        }
        ...
        attributeSymbol->attributeProcessorResolvedTypes.push_back(processorType);
        attributeSymbol->attributeProcessorCanonicalTypes.push_back(
            processorSymbol->scopePath.empty() ? processorSymbol->name
        ...
        attributeSymbol->attributeProcessorHookSymbols.emplace_back();
        ...
        if (phase == "pre" || phase == "post" || phase == "finally")
        {
            const std::string_view methodName =
                phase == "pre" ? std::string_view("Before")
            ...
            if (validHook && phase == "pre" && hook->parameters.size() == 1)
            {
                Ref<Type> receiverType =
                    hookType->paramTypes.empty() ? nullptr : unwrapAliasType(hookType->paramTypes.front());
                const bool receiverIsAny = receiverType && receiverType->kind() == TypeKind::Primitive &&
                                           receiverType.AsFast<PrimitiveType>()->name == "any";
                const auto receiverReference = receiverType && receiverType->kind() == TypeKind::Reference
                                                   ? receiverType.AsFast<ReferenceType>()
            ...
            const bool returnsBool =
                validHook && unwrapAliasType(hookType->returnType) == Compiler::get().getTypeContext().getBool();
            if (phase != "pre")
            ...
            if (!validHook)
            {
                ...
                {
                    WIO_LOG_ADD_ERROR(node.location(),
                                      "pre processor '{}' must declare Before with no arguments, receiver: any, or an "
                                      "immutable typed receiver view; it may return bool only as a unit-target guard.",
                                      processorName);
                }
                else
                {
            ...
            {
                attributeSymbol->attributeProcessorHookSymbols.back() = hookSymbol;
                attributeSymbol->attributeProcessorHookCppNames.back() =
                    codegen::Mangler::mangleFunction(std::string(methodName), hookType->paramTypes);
                if (returnsBool)
                    hookMode += "_guard";
        ...
        if (phase == "around")
        {
            ...
            Ref<Symbol> hookSymbol = hook && hook->name ? hook->name->referencedSymbol.Lock() : nullptr;
            Ref<FunctionType> hookType = hookSymbol ? hookSymbol->type.AsFast<FunctionType>() : nullptr;
            Ref<Type> proceedType =
                hookType && hookType->paramTypes.size() == 1 ? unwrapAliasType(hookType->paramTypes.front()) : nullptr;
            Ref<FunctionType> proceedFunction =
                proceedType && proceedType->kind() == TypeKind::Function ? proceedType.AsFast<FunctionType>() : nullptr;
            if (!hook || !hookType || hook->parameters.size() != 1 || !hookType->returnType || !proceedFunction ||
                !proceedFunction->paramTypes.empty() || !proceedFunction->returnType ||
                !hookType->returnType->isCompatibleWith(proceedFunction->returnType))
            {
                WIO_LOG_ADD_ERROR(node.location(),
                                  "around processor '{}' must declare 'fn Around(proceed: fn() -> T) -> T' (unit T may "
                                  "omit the return spelling).",
                                  processorName);
            }
            else
            {
        ...
        if (!validateMethod || !validateReturn || !validateReturn->value || !validateMethod->parameters.empty())
        {
            WIO_LOG_ADD_ERROR(node.location(),
                              "Validator '{}' must declare parameterless 'fn Validate() -> bool' with one compile-time "
                              "return expression.",
                              processorName);
            continue;
        }
        ...
        std::unordered_set<const Symbol*> activeSymbols;
        const auto foldedValidation =
            tryEvaluateStaticAttributeConstant(validateReturn->value, variableDeclarationsBySymbol_, activeSymbols);
        if (!foldedValidation ||
            (foldedValidation->type != TokenType::kwTrue && foldedValidation->type != TokenType::kwFalse))
        {
            WIO_LOG_ADD_ERROR(validateReturn->location(), "Validator '{}.Validate' must fold to a compile-time bool.",
                              processorName);
            continue;
        }
        ...
        if (const FunctionDeclaration* diagnosticMethod = findProcessorMethod("Diagnostic"))
        {
            ...
            const auto foldedDiagnostic = tryEvaluateStaticAttributeConstant(
                diagnosticReturn->value, variableDeclarationsBySymbol_, activeSymbols);
            if (!foldedDiagnostic || foldedDiagnostic->type != TokenType::stringLiteral ||
                foldedDiagnostic->isUnicodeString)
            {
                WIO_LOG_ADD_ERROR(diagnosticReturn->location(),
                                  "Validator '{}.Diagnostic' must fold to a compile-time string.", processorName);
                continue;
            }
