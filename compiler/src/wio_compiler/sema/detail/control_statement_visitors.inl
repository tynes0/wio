...
void SemanticAnalyzer::visit(IfStatement& node)
{
    ...
    const auto narrowedSymbolForNullComparison = [&](bool nonNullBranch) -> Ref<Symbol>
    {
        ...
        if (!binary || (binary->op.type != TokenType::opEqual && binary->op.type != TokenType::opNotEqual))
        {
            return nullptr;
        }
    ...
    if (node.matchVar.isValid())
    {
        if (node.condition->is<BinaryExpression>())
        {
            ...
            if (binExpr->op.type == TokenType::kwIs && isAnyType(lhsType) && isSupportedAnyCastTargetType(targetType))
            {
                ...
                auto varSym =
                    createSymbol(node.matchVar.value, targetType, SymbolKind::Variable, node.matchVar.loc, flags);
                currentScope_->define(node.matchVar.value, varSym);
                node.matchSymbol = varSym;
            }
            else if (binExpr->op.type == TokenType::kwIs && targetStruct)
            {
                ...
                flags.set_isMutable(true);
                auto varSym =
                    createSymbol(node.matchVar.value, refType, SymbolKind::Variable, node.matchVar.loc, flags);
                currentScope_->define(node.matchVar.value, varSym);
                node.matchSymbol = varSym;
            }
            else
            {
                WIO_LOG_ADD_ERROR(
                    node.matchVar.loc,
                    "Pattern matching 'fit' can only be used with the 'is' operator (e.g., target is Boss fit t).");
            }
        }
        else
        {
...
void SemanticAnalyzer::visit(WhileStatement& node)
{
    ...
    if (auto condType = node.condition->refType.Lock(); condType != Compiler::get().getTypeContext().getBool())
    {
        ...
        if (!allowsNumericSemantics(condType) && condType->kind() != TypeKind::Reference &&
            condType->kind() != TypeKind::Null)
        {
            WIO_LOG_ADD_ERROR(node.condition->location(),
...
void SemanticAnalyzer::visit(ForInStatement& node)
{
    ...
    auto createBindingSymbol =
        [&](const NodePtr<Identifier>& binding, const Ref<Type>& bindingType, ForBindingMode bindingMode)
    {
        SymbolFlags flags = SymbolFlags::createAllFalse();
        if (bindingMode == ForBindingMode::ValueMutable || bindingMode == ForBindingMode::ReferenceMutable)
            flags.set_isMutable(true);

        Ref<Symbol> bindingSym =
            createSymbol(binding->token.value, bindingType, SymbolKind::Variable, binding->location(), flags);
    ...
    if (node.iterable->is<RangeExpression>())
    {
        ...
        else if (getBindingMode(0) == ForBindingMode::ReferenceMutable ||
        ...
        else
        {
            ...
            createBindingSymbol(node.bindings[0], createLoopBindingType(rangeValueType, getBindingMode(0)),
                                getBindingMode(0));
        }
    }
    else if (iterableType && iterableType->kind() == TypeKind::Array)
    {
        ...
        if (node.bindings.size() == 1)
        {
            createBindingSymbol(node.bindings[0], createLoopBindingType(elementType, getBindingMode(0)),
                                getBindingMode(0));
        }
        else if (elementType && elementType->kind() == TypeKind::Struct)
        {
            ...
            if (structType->isObject || structType->isInterface)
            {
                WIO_LOG_ADD_ERROR(node.location(),
                                  "Destructuring in array loops currently supports component-like POD structs only.");
            }
            else if (node.bindings.size() == structType->fieldNames.size())
            {
                for (size_t i = 0; i < node.bindings.size(); ++i)
                {
                    ...
                    createBindingSymbol(node.bindings[i],
            ...
            {
                if (getBindingMode(0) == ForBindingMode::ReferenceMutable ||
                    getBindingMode(0) == ForBindingMode::ReferenceView)
                {
            ...
            else if (node.bindings.size() == 2)
            {
                ...
                else
                {
                    ...
                    createBindingSymbol(node.bindings[0], indexType, getBindingMode(0));
                    createBindingSymbol(node.bindings[1], createLoopBindingType(elementType, getBindingMode(1)),
                                        getBindingMode(1));
                }
            }
            else
            {
                WIO_LOG_ADD_ERROR(
                    node.location(),
                    "Component binding expects {} names, or {} with an index binding, but {} were provided.",
        ...
        {
            WIO_LOG_ADD_ERROR(node.location(),
                              "Array iteration supports either a single value binding or 'index | value'.");
        }
    }
    else if (iterableType && iterableType->kind() == TypeKind::Dictionary)
    {
        ...
        else
        {
            ...
            {
                WIO_LOG_ADD_ERROR(node.location(),
                                  "Dictionary keys are immutable. Use a value binding or 'view' for the key.");
            }
            else
            {
    ...
    else
    {
        ...
        WIO_LOG_ADD_ERROR(node.location(), "For-in loops currently require an array or dictionary type. Got '{}'.",
                          actualType);
    }
...
void SemanticAnalyzer::visit(ReturnStatement& node)
{
    ...
    if (node.value && resolvedExpectedReturnType && resolvedActualReturnType &&
        ...
        resolvedActualReturnType->kind() == TypeKind::Reference)
    {
        ...
        {
            WIO_LOG_ADD_ERROR(node.value->location(),
    ...
    {
        if (!currentFunctionReturnType_->isUnknown() && actualType && !actualType->isUnknown() &&
            !isAssignmentLikeCompatible(currentFunctionReturnType_, actualType))
        {
            ...
            {
                WIO_LOG_ADD_ERROR(node.location(), "Return type mismatch! Expected '{}', but got '{}'.",
                                  currentFunctionReturnType_->toString(), actualType->toString());
            }
        }
    }
    else
...
void SemanticAnalyzer::visit(UseStatement& node)
{
    ...
    auto getOrCreateNamespace = [&](const Ref<Scope>& targetScope, const std::string& name) -> Ref<Symbol>
    {
        ...
        auto nsSymbol =
            createSymbol(name, Compiler::get().getTypeContext().getUnknown(), SymbolKind::Namespace, node.location());
    ...
    if (auto importedNamespace = resolveImportedNamespace())
    {
        if (!node.aliasName.empty())
        {
            if (auto existingAlias = currentScope_->resolveLocally(node.aliasName))
            {
                ...
                WIO_LOG_ADD_ERROR(node.location(), "Symbol '{}' already exists and cannot be used as an import alias.",
                                  node.aliasName);
                return;
            }
    ...
    if (node.isStdLib)
    {
        WIO_LOG_ADD_ERROR(node.location(), "Standard library module 'std::{}' could not be resolved after merge.",
                          node.modulePath);
        return;
    }
    ...
    for (const auto& importedName : node.importedSymbols)
    {
        ...
        {
            WIO_LOG_ADD_ERROR(node.location(),
                              "Imported symbol '{}' from module '{}' could not be resolved after merge.", importedName,
                              node.modulePath);
            continue;
        }
    ...
    for (const auto& importedSymbol : importedSymbols)
    {
        ...
        if (auto existingSymbol = currentScope_->resolveLocally(importedSymbol->name))
        {
            ...
            WIO_LOG_ADD_ERROR(node.location(),
                              "Symbol '{}' already exists and cannot be directly imported from module '{}'.",
                              importedSymbol->name, node.modulePath);
            continue;
        }
